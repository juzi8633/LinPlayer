...
const (
	...
	SourceBangumi Source = "bangumi" // 公开 API,不要密钥(D362)
)
...
type Category struct {
	...
	// BangumiSeason Bangumi 榜:true = 只看本季开播的,false = 全站排名。
	BangumiSeason bool `json:"-"`
}
...
var Categories = []Category{
	{"anime_hot_week", GroupAnime, SourceDandan, "本周热门", sp("all/hot/week"), nil, false},
	{"anime_hot_month", GroupAnime, SourceDandan, "本月热门", sp("all/hot/month"), nil, false},
	{"anime_rising_week", GroupAnime, SourceDandan, "本周飙升", sp("all/rising/week"), nil, false},
	{"anime_new_current", GroupAnime, SourceDandan, "当季新番", sp("new-anime/hot/current-season"), nil, false},
	{"anime_new_previous", GroupAnime, SourceDandan, "上季新番", sp("new-anime/hot/previous-season"), nil, false},
	{"movie_trending_week", GroupMovie, SourceTMDB, "本周趋势", nil, sp("/trending/movie/week"), false},
	{"movie_popular", GroupMovie, SourceTMDB, "流行", nil, sp("/movie/popular"), false},
	{"movie_top_rated", GroupMovie, SourceTMDB, "高分", nil, sp("/movie/top_rated"), false},
	{"movie_now_playing", GroupMovie, SourceTMDB, "正在上映", nil, sp("/movie/now_playing"), false},
	{"tv_trending_week", GroupTV, SourceTMDB, "本周趋势", nil, sp("/trending/tv/week"), false},
	{"tv_popular", GroupTV, SourceTMDB, "流行", nil, sp("/tv/popular"), false},
	{"tv_top_rated", GroupTV, SourceTMDB, "高分", nil, sp("/tv/top_rated"), false},
	{"tv_on_the_air", GroupTV, SourceTMDB, "正在播出", nil, sp("/tv/on_the_air"), false},
	{"bangumi_rank", GroupAnime, SourceBangumi, "Bangumi 排名", nil, nil, false},
	{"bangumi_season", GroupAnime, SourceBangumi, "Bangumi 本季", nil, nil, true},
}
...
func Available() []Category {
	...
	for _, c := range Categories {
		if (c.Source == SourceDandan && AnimeConfigured()) || (c.Source == SourceTMDB && VideoConfigured()) || c.Source == SourceBangumi {
			out = append(out, c)
		}
	}
	return out
...
func fetchWith(ctx context.Context, categoryID string, forceRefresh bool, dandanID, dandanSecret, tmdbKey string) ([]Entry, error) {
	...
	switch cat.Source {
	case SourceBangumi:
		list, err = fetchBangumi(ctx, cat, time.Now())
	case SourceDandan:
