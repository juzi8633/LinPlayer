...
func ImageOrigins() []string {
	out := []string{
		...
		lpsync.BangumiImgMirror, // Bangumi 榜的封面走反代
	}
...
func RegisterCommands() {
	// 登记进图片通道:这批不在账号表里,必须走能扛住整表重建的那个入口(见 account.AllowImageOrigin)
	for _, o := range ImageOrigins() {
		account.AllowImageOrigin(o, http.Header{})
	}
