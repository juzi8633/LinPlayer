...
func TestCategories自洽(t *testing.T) {
	...
	for _, c := range Categories {
		...
		case SourceBangumi:
			if c.TMDBPath != nil || c.DandanPath != nil {
				t.Fatalf("%s 是 Bangumi 源,不该带别家的路径", c.ID)
			}
		default:
...
func TestAvailable没凭据就不亮(t *testing.T) {
	...
	bangumi := 0
	for _, c := range got {
		if c.Source == SourceBangumi {
			bangumi++
		}
	}
	if bangumi != 2 {
		t.Fatalf("Bangumi 榜不要密钥,应当总是亮着(D362),实际 %d 个", bangumi)
	}
	if !AnimeConfigured() && !VideoConfigured() && len(got) != bangumi {
		t.Fatalf("两源都没凭据,却亮了 %d 个要凭据的分类", len(got)-bangumi)
	}
	for _, c := range got {
...
// Bangumi 榜:按排名顺序出、中文名优先、本季榜带上开播年月、封面走反代(D362)。
func TestBangumiRanking(t *testing.T) {
	fresh(t)
	var gotQuery url.Values
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotQuery = r.URL.Query()
		w.Write([]byte(`{"data":[
			{"id":326,"name":"攻殻機動隊","name_cn":"攻壳机动队","date":"2004-01-01","eps":26,"images":{"common":"https://lain.bgm.tv/r/400/pic/a.jpg"},"rating":{"rank":1,"score":9.2}},
			{"id":7,"name":"原名","name_cn":"","images":{"large":"https://lain.bgm.tv/pic/b.jpg"},"rating":{"rank":2,"score":0}}]}`))
	}))
	defer srv.Close()
	old := lpsync.BangumiAPIOfficial
	lpsync.BangumiAPIOfficial = srv.URL
	defer func() { lpsync.BangumiAPIOfficial = old }()

	list, err := Fetch(context.Background(), "bangumi_rank", true)
	if err != nil || len(list) != 2 {
		t.Fatalf("Bangumi 排名: %v %v", list, err)
	}
	if list[0].Title != "攻壳机动队" || list[1].Title != "原名" || list[0].Rank != 1 {
		t.Fatalf("中文名优先、没有中文名用原名、按顺序排名: %+v", list)
	}
	if list[1].Rating != nil {
		t.Fatal("0 分(没人评)不该显示成 0 分")
	}
	if list[0].ImageURL == nil || strings.Contains(*list[0].ImageURL, "lain.bgm.tv") {
		t.Fatalf("封面应改写到反代: %v", list[0].ImageURL)
	}
	if gotQuery.Get("sort") != "rank" || gotQuery.Get("type") != "2" || gotQuery.Get("year") != "" {
		t.Fatalf("全站排名不该带年月: %v", gotQuery)
	}
	if _, err := Fetch(context.Background(), "bangumi_season", true); err != nil {
		t.Fatal(err)
	}
	if gotQuery.Get("year") == "" || gotQuery.Get("month") == "" {
		t.Fatalf("本季榜要带开播年月: %v", gotQuery)
	}
}

func TestSeasonStart(t *testing.T) {
	for m, want := range map[time.Month]int{time.January: 1, time.March: 1, time.April: 4, time.September: 7, time.December: 10} {
		if _, got := seasonStart(time.Date(2026, m, 15, 0, 0, 0, 0, time.UTC)); got != want {
			t.Errorf("%v → %d,期望 %d", m, got, want)
		}
	}
}
