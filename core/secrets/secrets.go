...
// 这三个由 `-ldflags -X linplayer/core/secrets.xxx=...` 在链接期灌进来。
...
// 字面量里的 tmdb-ranking 是历史名字,改它 = CI 里的密文全部解不开。
...
// TMDBKey TMDB 密钥(密文注入,排行榜影视榜用)。空 = 未配置。
func TMDBKey() string { return decrypt(tmdbKeyEnc) }

// TMDBConfigured 这个构建带没带 TMDB 密钥。判据是**解得出明文**:密文非空但解不开时
// 说「配了」,请求就会带着空 key 出去,拿回一句看不懂的鉴权失败。
