...
// ProxyBase 同步代理基址。空 = 这个构建没配。注入值在 core/system(反馈通道共用同一个代理)。
func ProxyBase() string { return system.ProxyBase() }
...
func ProxyHeaders() map[string]string {
	...
	return map[string]string{"X-LinPlayer-Key": system.ProxyKey()}
}
