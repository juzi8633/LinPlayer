...
#                                                              选集轨道:虚拟化 + 一路滑过 200 集量帧间隔
#   LP_BAND=1 bash scripts/selfcheck-win.sh band detail:s1
#                                                              详情页头图带:通栏 24:8 + 返回/动作组
#   LP_CHIPS=1 bash scripts/selfcheck-win.sh chips grid:lib-1
#                                                              媒体库:已选筛选片 + 点 ✕ 去掉
#   LP_SEARCH=某 LP_WAIT=16 bash scripts/selfcheck-win.sh search
#                                                              搜索浮层:Ctrl K 唤起 / 不跳页 / Esc 收起 / 历史片
#                                                              (这条要跑满 8 秒,默认 6 秒会把最后一行截掉)
#   LP_VIEW=1  bash scripts/selfcheck-win.sh view grid:lib-1
#                                                              媒体库版式开关:海报网格 ⇄ 列表
#   LP_TOOLS=1 bash scripts/selfcheck-win.sh tools
#                                                              内容区右上两颗:搜索 / 刷新 / 滚动淡出
...
[ -n "${LP_SRV2:-}" ] && SRV2=',{ "server": "http://127.0.0.1:1", "token": "t2", "user_id": "u2",
    "user_name": "第二个用户", "name": "自检用第二台", "remark": "备注:第二台",
    "lines": [], "active_line": 0 }'
cat > "$BIN/userdata/config.json" <<JSON
{
  "device_id": "linplayer-selfcheck",
...
LP_CORELOG="${LP_CORELOG:-}" LP_SELFCHECK=1 LP_SELFCHECK_MENU="${LP_MENU:-}" LP_SELFCHECK_COUNT="${LP_COUNT:-}" LP_SELFCHECK_BOOM="${LP_BOOM:-}" LP_SELFCHECK_VERSION="${LP_VER:-}" LP_SELFCHECK_REPAINT="${LP_REPAINT:-}" LP_SELFCHECK_HERO="${LP_HERO:-}" LP_SELFCHECK_NAVHOVER="${LP_NAVHOVER:-}" LP_SELFCHECK_GLYPH="${LP_GLYPH:-}" LP_SELFCHECK_COLLAPSE="${LP_COLLAPSE:-}" LP_SELFCHECK_FILL="${LP_FILL:-}" LP_SELFCHECK_SIDEBAR="${LP_SIDEBAR:-}" LP_SELFCHECK_SRVMENU="${LP_SRVMENU:-}" LP_SELFCHECK_RAIL="${LP_RAIL:-}" LP_SELFCHECK_RAILSTRESS="${LP_RAILSTRESS:-}" LP_SELFCHECK_EPVIEW="${LP_EPVIEW:-}" LP_SELFCHECK_HEROBAND="${LP_BAND:-}" LP_SELFCHECK_FILTERCHIPS="${LP_CHIPS:-}" LP_SELFCHECK_SEARCHOVERLAY="${LP_SEARCH:-}" LP_SELFCHECK_VIEW="${LP_VIEW:-}" LP_SELFCHECK_TOOLS="${LP_TOOLS:-}" LP_SELFCHECK_CHROME="${LP_CHROME:-}" LP_SELFCHECK_OSDFADE="${LP_OSDFADE:-}" LP_SELFCHECK_RESUME="${LP_RESUME:-}" LP_SELFCHECK_RECLICK="${LP_RECLICK:-}" LP_SELFCHECK_SRVICON="${LP_SRVICON:-}" LP_SELFCHECK_THUMB="${LP_THUMB:-}" LP_SELFCHECK_AVSYNC="${LP_AVSYNC:-}" LP_SELFCHECK_STUTTER="${LP_STUTTER:-}" LP_SELFCHECK_PICK="${LP_PICK:-}" LP_SELFCHECK_HOVERLAYOUT="${LP_HOVERLAYOUT:-}" LP_SELFCHECK_PAUSE="${LP_PAUSEAT:-}" LP_SELFCHECK_PANEL="${LP_PANEL:-}" LP_SELFCHECK_DOWNLOAD="${LP_DL:-}" LP_SELFCHECK_TOAST="${LP_TOAST:-}" LP_SELFCHECK_KEYS="${LP_KEYS:-}" LP_SELFCHECK_HOME="${LP_HOME:-}" LP_SELFCHECK_HOMESET="${LP_HOMESET:-}" LP_SELFCHECK_WATCHED="${LP_WATCHED:-}" LP_SELFCHECK_PAGE="$PAGE" LP_SELFCHECK_MAXIMIZE="${LP_MAX:-}" LP_SELFCHECK_PLAYER_DRILL="${LP_DRILL:-}" LP_SELFCHECK_SCROLL="${LP_SCROLL:-}" LP_SELFCHECK_SOURCE="${LP_SRCKIND:-}" LP_SELFCHECK_CATALOG_DETAIL="${LP_CATDETAIL:-}" LP_SELFCHECK_SHADER="${LP_SHADER:-}" LP_SELFCHECK_INTERP="${LP_INTERP:-}" LP_SELFCHECK_INTERP_TRT="${LP_INTERP_TRT:-}" LP_SELFCHECK_REORDER="${LP_REORDER:-}" "$BIN/LinPlayer.exe" > "$ROOT/build/app.log" 2>&1 &
# 播放页要等起播 + 解码,别的页 6 秒够
...
# LP_FLICKER=帧数 单进程连拍,量快速闪烁(shot-burst 每张要起一次 PowerShell,太慢)
if [ -n "${LP_FLICKER:-}" ]; then
  PYTHONUTF8=1 python "$ROOT/scripts/flicker-probe.py" "$LP_FLICKER" "${LP_FLICKER_MS:-25}"     "${LP_FLICKER_Y0:-0.925}" "${LP_FLICKER_Y1:-0.99}" "${LP_FLICKER_WHAT:-按钮行}"
fi
# LP_BURST=张数 连拍,量「暂停时画面动没动」。见 scripts/shot-burst.sh
if [ -n "${LP_BURST:-}" ]; then
  bash "$ROOT/scripts/shot-burst.sh" "$SHOT" "$LP_BURST" "${LP_BURST_MS:-120}" "${LP_BURST_ZONE:-}" "${LP_BURST_WHAT:-画面}"
fi
