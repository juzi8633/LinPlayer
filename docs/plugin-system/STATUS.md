...
| 三端同一组示例页截图**一致** | ✅ 判据不是像素相同(三端主题 / 密度 / 字体本来就不同),是**没有哪一端把组件悄悄降级成占位**:`PluginUiFocusTest` 喂 18 个组件、零占位;两端未知组件都记 warn,桌面自检抓这一行 | 三张截图 + 该测试 |
| TV 1000 项 VirtualList ≥50fps | ✅ 模拟器实测:80 次下键滚到第 81 项,**606 帧、janky 0.66%(4 帧)**,50th 21ms / 99th 34ms。核心层那半:一次局部更新 2 条 op(20 项与 1000 项相同)、首帧只建 24 个节点。⚠️ 跑的是 swiftshader 软渲染的模拟器,**这组数字不替真 TV 盒子签字** | `dumpsys gfxinfo`;`rt/ui_bench_test.go` |
| 插件页首帧 < 300ms | ✅ 壳侧实测,冷路径:桌面 **291ms**、手机 **284ms**、TV **114ms**(46 节点)/ **120ms**(1000 项那页,173 节点);TV 热路径(同进程再进一次)**59 / 152 / 157ms**。核心层那半 7ms | 日志 `[插件UI] … 首帧 N ms`;`TestUI首帧预算` |
...
| 插件页一进去就崩(异常,不是画不出来) | `VirtualList` 没给高度时,可滚动容器嵌在可滚动容器里高度无界 |
| TV 上一千项列表**整页空白**(标题在、行全没了) | 回调实参里的 Map 落到 `toString()`,JS 收到字符串 `"{from=0, to=24}"`,`r.from\|0` 算成 0 → 窗口变 0..0 |
| TV 上一千项列表**一行都滚不动**(80 次下键只出 10 帧) | 项全是 Text,没有一个能吃焦点 —— 整页没落点,焦点全程停在侧边栏 |
| 自检永远只量得到冷路径 | `-e lp_page` 只在 `onCreate` 收,热启动送进来没人接 |
