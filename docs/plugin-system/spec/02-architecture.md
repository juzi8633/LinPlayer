...
⚠ **预算不是硬上限**(D553):goja 每条字节码查一次中断标志,但**单次原生大操作**(超大数组 `join`、超大字符串 `repeat`)中途不查 —— 要跑完这一步才停。实测同一段 3000 万元素 `join`:桌面 2.8 秒,Android 模拟器 p50 13.1 秒、max 35.0 秒,慢设备上一次这种操作就能越过 30 秒的 `BudgetData`。不再加机制,同一类问题由内存看门狗(2.3.2,D141)兜住。下表的数字读作「**这一步跑完就停**」,不是「绝不超过」。
...
打断延迟已实测(`09-goja-interrupt-spike.md`):纯 JS 死循环桌面 <1ms、Android <2.5ms。另补一处:回溯类正则落到 regexp2,而它默认不超时也不查 goja 的中断标志,一条坏规则能把运行时永久卡死 —— `core/plugin/rt` 在包初始化把 `regexp2.DefaultMatchTimeout` 钉 1 秒(D552),代价是超时的那次匹配被当成「不匹配」。
