...
- **视频壁纸**:进播放页、应用到后台、系统省电时暂停(三条是**与**,任一要求暂停就暂停);低分辨率播放,不占播放用的 mpv 实例。
  实现方式已由 spike 定案(D564,见 `docs/research/plugins-v2/12-video-wallpaper-spike.md`):
  **安卓用 ExoPlayer + 独立 `SurfaceView`(现在就能做);桌面 2.0.0 不做** ——
  一个进程只能有一条 GL 通道,而核心层的 render context 是全局单例,
  桌面视频壁纸和 `<Player>`(D561)是同一件地基。桌面降级成静态图 + Canvas/着色器动态壁纸;
  挑了视频壁纸包时显示封面帧并说清这一端只放第一帧。
