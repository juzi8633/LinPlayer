...
<p align="center">
  ...
  <img src="https://img.shields.io/endpoint?url=https://291277.xyz/sentry/users" alt="Active Users">
...
- **配布版（Windows / Linux / Android / Android TV）には匿名のクラッシュ報告（Sentry）が含まれます**：クラッシュ時にスタックトレース、
  OS とアプリのバージョン、ランダム生成の匿名インストール識別子（アクティブ端末数の集計のみに使用）を送信します。
  送信前にユーザーフォルダのパスを `~` に、URL 内の token / api_key を伏せ字に置き換えます。パフォーマンス計測・画面録画は行いません。
  ご自身でビルドした版には送信先が含まれず、一切送信しません。
- **クラッシュ後の次回起動時、クラッシュ情報と直近のログを自動で開発者に送信します**。エラー表示の「反馈」、設定の「发送日志给开发者」も同じ経路です。
  送信前にサーバーアドレス・アカウント・トークン・ユーザー名を削除します。
- 私たちは**個人を特定できる情報を一切収集しません**：アカウント、パスワード、Cookie、トークン、
  サーバーアドレス、ライブラリの内容、視聴履歴、IP アドレスのいずれも収集せず、**画面録画も行動追跡も行いません**。
- クラッシュデータは**販売・共有したり、広告その他いかなる商業目的にも使用しません**。
  Sentry の保持ポリシー（既定で最長 90 日）に従って自動削除されます。
