...
- 按 UA 拉黑的服:线路测速没带 UA,能播的线路全显示「不通」— 2026-09-15
...
## 环形缓存的「钉住文件头」有两半,少一半等于没钉 — 2026-09-04

现象:进度条缩略图一句「打不开」,而 `player.status` 的 `cached` 报着 30% 已缓存、
正片放得好好的。查下去 spans 是 `0.70-1.00` —— **整条时间轴没有头**。

`slotOf` 早就把分段 0 / last-1 / last 放进了环外的三个固定槽,永不轮换。
那是第①半。**第②半 2026-09-04 才补上**:

> ffmpeg 打开一个 mp4,先发 `Range: bytes=0-` 读文件头,**只读几百 KB**
> 认出 moov 的位置就把这条连接关了,另开一条从 moov 处读。
> 而代理这边正在为它拉整个 4MB 的分段 0 —— 连接一关 `s.done` 就闭,
> `fetchCtx` 当场取消,**分段 0 半途而废、不落盘**。

所以:**钉住的那几段,下到一半也不许跟着连接一起取消**(`serve.go` 的 `pin` 分支)。

★ 之前还修过一个**同根不同枝**的:「首段不对齐时只拉残段」——
  那一条挡的是「从块中间开始」,挡不到「连接活得比这一段短」。
  两条都由 `disk.pinned(c)` 判,但作用在两个不同的地方,**都要有**。
★ 代价有界:被丢下的连接最多多拉 ≤3 段(12MB),而且每条流只发生一次
  (第二次 `disk.has(c)` 直接跳过)。
★ 仍然要听 **origin 的关闭**(`o.stop`),不能听连接的:换片 / 停播会删缓存文件,
  那时候还在写盘就是写一个已经不存在的文件。
★ 护栏要成对:光写「钉住的不许取消」的话,一个「所有段都不许取消」的实现也照样绿,
  而那种实现下用户跳一次进度条,每条被丢下的连接都要把 12MB 拉完才罢休。
  所以 `TestC30b` 反过来钉「没钉住的段该停就停」。
★ 造这个形状**必须让上游慢一点**(测试里 `up.slow = 8ms`):本地环回一瞬间就拉完了,
  「读一点就断」根本发生不了 —— 又是一条「夹具造不出形状,判据就验不出对错」。

## 按 UA 拉黑的服:线路测速没带 UA,能播的线路全显示「不通」— 2026-09-15

用户反馈「某台服播放不了」。真服实测它是 **UA 黑名单**(不是白名单):
...
★ **Go 的 `http.NewRequest` 不设 UA 不是「不发」,是发 `Go-http-client/1.1`** —— 和 reqwest 那条相反,但结果一样被拉黑。
  ...
  自己 `hc.Do(req)` 的必须手动设。
★ 查这类服先分清**是 IP 还是 UA**:本机走代理时同一台服回的是 307 跳 bing「不被允许的 IP」,
  和 UA 无关;绕开代理后才看到真正的 UA 规则。别把代理出口被拦当成我们的 bug。
★ 本机 mihomo 下每条新连接 TLS 握手 ~5s(对照 Google 同样 5s),**那是本机代理,不是服务器慢**。
★ 失效条件:服主改规则。线路 / 域名 / 账号**一律不写进仓库**。
