...
## 播放页手势:方向要锁死,不能每帧重判(2026-09-08)

用户报「上下滑动和左右滑动区分不开,左右滑顺带把上下也滑了」。
根因在判据上:上一版是**每一帧**比 `|drag.x|` 和 `|drag.y|` 谁大。手指划出的线
几乎不可能是正的,于是一次左右滑里夹着几十帧被判成上下 —— 音量就是这么被带走的。

★ 判据换成**累计**位移谁先攒够 28dp(`lockAxis`),而且定了这一次手势就不再改。
★ 主方向还要比副方向多出一截(1.7 倍):45° 斜划**什么都不该触发**,
  而不是随机落到其中一边。
★ 攒够的那一段 slop **不算进调节量**:算进去的话方向刚定下来的一瞬间会跳一格,
  手感上就是「一碰就窜」。
★ 左右半屏(音量/亮度)要在 `onDragStart` 就认定。按当前手指位置每帧判的话,
  手指划过屏幕中线时会从调音量变成调亮度。

竖滑的行程也从「一屏 = 满量程」放到 1.4 屏 —— 看片时手本来就搭在屏幕上,
一屏满量程稍微一动就是一大格,这是「太敏感」的另一半。

**钉法**:`lockAxis` 是纯函数,`LogicTest` 三条(没攒够不定 / 斜划不定 / 定了是哪边)。
反向注入换回「这一帧谁大」当场红。

## 底栏第三格换成收藏(2026-09-12)

用户:「将服务器 Tab 换成 收藏 Tab……增加 收藏的排序功能」。
服务器一台一台加完就不再动了,而收藏是每天要看「哪部更新了」的地方。
服务器挪去聚合页那排快捷入口 —— 一进一出,不留重复入口也不留死入口。

**换 Tab 会顺手改掉两处不在 Tab 栏里的代码,而且都不报错:**

- 首页「管理服务器」原来走 `switchTab(Route.Servers)`。服务器不再是 Tab 之后,
  `switchTab` 会把它当成一根平级栈弹到起点 —— 返回键回不到首页。要改回 `navigate`。
  (`HomePage` 上那段「`navigate(Route.Servers)` 是错的」的注释同时作废,已删。)
- 聚合页那颗「收藏」原来是 `navigate`,会把收藏页压进聚合这根栈里,
  于是 Tab 高亮跳到第三格而人还在第二格。

收藏页从「有返回键的子页」变成「Tab 根页」:`onBack` 要去掉,底部留白靠
`LocalTabClearance` 自动跟上(`contentInsets()` 里已经算了)。

排序档位切换必须**带进重拉的判据**里 —— 只判「手里有没有结果」的话换档位会被
当成「已经有了」直接跳过,和媒体库那个「筛选了不刷新」同一个形状。
