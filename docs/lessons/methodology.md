...
## 判据失效的六种形状(2026-09-21 一轮里全撞上了)

这一轮做插件系统阶段 ②~⑤,每一条都是**先绿后红**才发现的 —— 绿的时候看起来毫无异常。

1. **`go test` 的缓存看不见非 Go 的输入。**
   有几条测试跑的是插件的 `.ts` 源文件(拿真文件编出来在 goja 里跑)。改坏 TS 再跑,
   回的是 `ok (cached)` —— 门禁全绿。`check-core.sh` 的 `go test` 现在钉着 `-count=1`,
   注释里写了不许去掉。**判据的输入不在 Go 里时,缓存就是一层假绿。**

2. **断言的子串同时出现在失败路径上。**
   同步插件的判据是「插件日志里出现 scrobble 这个词」,而插件对不上条目时会记一行
   「scrobble 跳过:这一条没有 TMDB id」—— 那行里正好有这个词。于是**记账一次都没发生**,
   测试一直绿。判据改成「`trakt.request` 真的被调到了」之后,反向注入当场红。
   **量日志文案 ≠ 量行为。**

3. **「至少有几项」挡不住「删了一项」。**
   D11 的 mpv 脱敏清单判据写的是 `len(names) < 8`,而表里有 16 项 ——
   删掉 `http-header-fields` 和 `loadfile` 之后四条用例全绿,而那两行正是
   Authorization 与「插件自己拼地址去播」两个口子。改成**逐名对账**:
   加项不用改判据,**删项当场红**。

4. **门禁只查一个方向。**
   `check-bindings` 查了「Go 里有而文档没有」(野命令),没查反过来。
   于是 5 条命令只写进文档没注册,绑定层照样生成方法,壳调下去才报「没有这条命令」。
   **契约类门禁必须两个方向都红。**

5. **声明了、挂上了、没人调。** 这一轮抓到三处,表现全是「调了没反应」:
   · `nav.onBack` 订的是一个**没人发**的事件,返回值还被丢掉;
   · `definePlugin({ wallpaper })` 给的初始壁纸**从来没人调**;
   · `playerOverlays` 贡献点两端都没有挂载点,而核心层的按键通道接得好好的。
   共同点:每一层看自己那一段都是对的。**判据要跨层走一遍真实路径**,
   只验「这一层挂上了」永远发现不了。

6. **反向注入本身也会失手。**
   验 XMLTV 时区时,假数据用的是 `+0000` —— 符号搞反了也算不出差别;
   而判据只找节目标题,时区差 16 小时那一档会落到「接下来」那一行上,标题照样出现。
   两处都改掉(`+0800` + 连「正在播:」一起判)之后才真的红。
   **注入之后没红,先怀疑判据,别急着庆幸。**

配套的一条:**红线门禁的第一版滥报到没法用**(3195 条,`java.io` / `androidx.tv` /
`proguard-rules.pro` 全是命中)。门禁做成「第一天就红」的话,第二天起就没人看了 ——
存量进欠账清单(只存哈希,不存值),新增的才红。
