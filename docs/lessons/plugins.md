...
真接通的只有 13 个(2026-09-21 补了 `pageTakeovers` 和 `searchActions`);
`hooks` / `commands` / `menus` 接了一半;其余一行都没接。
...
第 5 条(三个壳都得真去调)**由源文件自己声明管哪些命令** —— 文件里写「门禁:三端都要调」
这行标记才算数。原来写死 `anchors.go` 一个文件名,新开一个文件注册的命令会悄悄漏出去
(`searchActions` 差点就是)。两关是绑着的:`contribPoints` 的落点如果是条 `plugin.*` 命令
却不在第 5 条的名单里就红 —— 否则标记被谁删了,第 5 条只会少查几条、照样全绿。

`pageTakeovers` 值得单独记一笔,因为它是「摆着不生效的控件」最完整的一个标本:
插件页的「接管位」标签**能列出来、能选、选完还弹「重启应用后生效」** ——
而没有任何一个壳会去画插件那一版。三层 UI 都写了,只差最后一段。
2026-09-21 补上(D586):`plugin.pageTakeovers` 只列**选中的、且插件还启用着的**,
三个壳画官方页之前问一次。两条边界是测出来的,不是想出来的:
**没选时必须是空的**(照「谁声明了就算谁接管」写会当场生效,用户没同意过),
**停用要还回去**,而停用和安装一样是重启才生效(D170)—— 第一版测试断言「停用后立刻还」,红了,
红得对:它测的是不存在的语义。

`searchActions`(D587)同样是两半:**只接「列出来」那一半,搜索页上就是一排点了没反应的
按钮**,比没有更糟。第一版还顺手开了一条 `plugin.runCommand`,
**写完才发现 `source.runCommand` 早就在那儿了**(它本来就是给菜单项和设置按钮跑插件命令用的,
不限于数据源)—— 已经删掉。新开一条同名同义的命令,代价是以后每个人都要猜该用哪个。
超时口径也是这条的关键:搜索框边打边出,**2 秒不回就当这个插件没给** ——
而且必须并发问,串着问的话这一串的总时长会摊在用户敲的每一个字上。
