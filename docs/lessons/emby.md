...
- 阈值字段的零值含义是「全都看完了」 — 2026-09-03
- 「按 X 定制」的开关必须有一处能看到全部 X — 2026-09-03
...
用户口径:「我们不做转码流,也不会去请求转码流」。
`ResolveStream` 里那条 `case nonEmpty(ms.TranscodingURL)` 已经删掉;
没有 `DirectStreamUrl` 时直接拼 `static=true` 的原流(原来那个兜底分支)。

**这不只是口味。** 转码 URL 是分段流,套不了字节代理 —— 一旦落到那条分支:

- 详情页预热作废
- 多线程加载作废
- **进度条缩略图整个不可用**(`localSource()` 拿不到本地字节,直接返回 `none`)

而且一条错都不报。用户同一轮里说的「我们不做转码流」和「还是没有缩略图」是**同一件事**。

★ 请求侧本来就没要过转码(`DeviceProfile` 里 `TranscodingProfiles: []`、
`DirectPlayProfiles` 声明啥都能直连),缺的只是**响应侧不要接**这一半。

★ 测试要卡在**两处**:`play_method` 得是 DirectStream,而且**地址里不能出现
服务器那条转码路径** —— 只断言前者的话,把 URL 照抄成转码地址、只把标签改成
DirectStream 也能过,那正是「界面在撒谎」那一类。

### 观看阈值:一个数字管两件事 — 2026-09-03

用户:「做一个观看阈值的功能,用户设置看到多少就标记为已观看,
以后再看这集就直接从头开始播放即可」。

`Prefs.WatchedThresholdPercent`(默认 90,下限 50)同时管:

1. **写**:停播时越过阈值就明着调 `emby.setPlayed` —— **不能只靠 ReportStopped**,
   服务器按**它自己**的阈值判(Emby 默认 90,fork 各改各的),
   只靠它的话设置页那个数字对用户看到的「已观看」标记毫无影响,是个设了没反应的开关。
2. **读**:起播时若续播位置已越过阈值,`resumeSecs` 归零(从头放)。

★ 两件事共用**同一个**值是刻意的:分成两个会出现「标了已看完却仍从 97% 续播」
这种自相矛盾的状态,而且没人看得出是哪儿设错了。

★ 下限 50 不是随手定的:再低就不是「看完」,看一半退出会被标已看完,
而那等于**把续播位置丢掉**。

★ 老配置里没有这个键 → Go 解出来是 0 → 含义是「放第一帧就算看完」,
每一集刚起播就被标已看完。`ParsePrefs` 从 `DefaultPrefs()` 起手挡住缺字段,
`Clamped()` 挡住越界值,两条各有一条测试。


### 手动标「已看」只写了服务器,本地记录纹丝不动 — 2026-09-03

`emby.setPlayed` 只调服务器,**从来没碰过本地观看记录**。两条静默的错路:

- **跨服续播**:本地记录还停在旧进度上 → 换一台服务器接着看,会从那个旧位置起播
- **跨服回传**:把那个旧进度写回**别人的服务器**

两条都不报错,用户只会觉得「标了没用」。

★ 修法上有个包依赖的坑:`core/history` **import 了** `core/emby`
(`candidate.go` 要 `emby.Item` 才折得出候选),所以 emby 不能反过来 import 它。
留一个回调槽 `emby.OnPlayedChanged`,由 `history.RegisterCommands` 填上。

★ 取条目**必须带 `HistoryFields`**:少了 ProviderIds / PresentationUniqueKey / Path,
指纹会静默降级到「剧名+季集号」,记录落到一个和播放时**不同的** canonicalKey 上 ——
本地存了两条,而续播读的是另一条。

### 阈值字段的零值含义是「全都看完了」 — 2026-09-03

`isPlayed(pos, runtime, threshold)` 里 `threshold = 0` 的字面含义是
**「位置 ≥ 0 就算看完」** —— 每一条落进来的记录都是已看完。

而 0 从来不是谁的本意,它是**「调用方忘了填这个字段」**的样子(Go 的缺省是零值)。
把「忘了填」翻译成「全都看完了」是这一层最贵的一种沉默:
用户的库会在某次重构之后一夜之间全变成已看,而没有任何东西报错。

现在 `threshold <= 0` 一律返回 false,配一条注入验过红的测试。

★ 同源的一条:`Prefs.WatchedThresholdPercent` 的 0 也被 `Clamped()` 挡回 90。
**同一个零值陷阱在一条链上出现两次**,两处都得堵 —— 只堵一处的话,
另一处的调用方仍然能把 0 传进来。

### 「按 X 定制」的开关必须有一处能看到全部 X — 2026-09-03

合集栏的开关第一版只作用于**当前登录**那台服务器,设置页也只画那一行。
用户在 A 服关掉、到 B 服看见还在,只能靠一台台切过去才知道自己设了什么。

改成每台服务器一行(`prefs.getHomeSettings` 回 `servers` 数组),
写的时候明着送 `server` 字段。

★ 送 `server` 的另一半是**核心层必须校验它是已知账号** ——
静默收下一个不存在的键,那一条谁也看不到、谁也删不掉,而它一直在生效。
...
### `seasonEpisodes` 回的是 `{items,total}`,不是裸数组 — 2026-09-06

**症状**:手机端剧集详情页和播放页「选集」面板**一集都不显示**,一条错都没有。

**根因**:`emby.seasonEpisodes` 返回 `*Page`(`{"items":[…],"total":N}`),
而两处都写成 `Item.list(结果)`,`arr()` 对 JsonObject 返回空表 ——
「这一季确实没有集」和「解析形状挑错了」在界面上长得一模一样。
PC 端同一条命令是按 `items` 取的,所以只有手机端中招。

**为什么容易挑错**:核心层**故意**两种形状并存 ——
要总数/翻页的走 `listItemsPage`,其余走裸数组(`commands.go` 有注释说明)。
`check-android-args.py` 只对账**参数名**,返回形状没有任何东西守。

**改法**:补在 `Item.list` 一处(两种形状都吃),不补三个调用点 ——
下一个新页面照样会挑错。JVM 单测钉住,反向注入验过会红。

**顺带**:`emby.seriesSeasons` 回的是 `[]SeasonInfo`,字段是 `id`/`name`,
拿 `Item.list` 解得出来(只是 `type_` 为空),所以季那一栏当时是「能出但被藏了」——
手机端写的是 `if (seasons.size > 1)`,单季剧整条不画,而单季是最常见的情况。


---

### 服务器图标:不校验内容就会把 HTML 缓存住 — 2026-09-06

**症状**:「确认站点是有图标的」,而软件里永远是那个内置的通用图标,一个字不报。

**根因两层**:
1. `sniffMime` 认不出格式时**一律返回 `image/png`**。反代 / SPA 常把
   `/web/touchicon.png` 这类不存在的静态文件回成一份 **200 的 index.html** ——
   那坨 HTML 被当成 png **落盘缓存**。
2. `IconGetAny` **先查缓存**。缓存一旦被污染,后面几条候选地址**一条都不会再试**。
   两端(Avalonia `Bitmap` / 安卓 `BitmapFactory`)解不开就各自 catch 掉,零日志。

**改法**:落盘前 `sniffImage` 判「确实是一张图」,不是就判失败让候选继续往下走;
SVG 也判失败(两端都解不开,认它等于换一个静默失败);每条候选失败都记一句 warn。
候选表补 `/apple-touch-icon.png` 与**根上的** `/favicon.ico` ——
反代把 Emby 挂在子路径下时 `/web/...` 全 404 而根上那条好好的。

**判据**:测试里让假服务器对 touchicon 回 200 HTML、对 favicon.ico 回真 ico,
断言拿到的是那张 ico **且缓存里躺着的也是它**。反向注入验过会红
(base64 开头是 `PCFkb2N0eXBl` = `<!doctype`)。
...
## 「测试连接」只探公开信息等于没测(2026-09-11)

`account.testConnection` 原来只打 `/System/Info/Public`。那个端点**不需要凭据** ——
于是地址填对、密码填错的服务器照样报「连上了」,用户点「登录」才发现不行,
而那两句话是自相矛盾的。

改成:给了用户名就**真登一次**(`emby.Login`),登完立刻 `Logout` ——
不落库、不切活跃会话(`account_test.go` 里那条断言钉着)。

★ 报错要翻成**用户照着能做下一步**的一句话,放在 `Msg` 里,真实原因降级进 `Detail`:

| 状态码 | 给用户看的 |
|---|---|
| 401 / 403 | 账号或密码不对,请重新输入账号密码。 |
| 404 | 这个地址上没有 Emby 接口,检查一下服务器地址(常见是少了 /emby 或端口写错)。 |
| 5xx | 服务器自己出错了(HTTP xxx),过一会儿再试。 |
| 连不上 | 连不上这台服务器:**原样带上真实原因** —— 自签名证书、DNS 解析不了、端口不通是三件事,而它们都长成「连不上」 |

★ 结果走**弹窗**不走页面上那行小字:小字在按钮附近,填完密码一弹键盘就被顶出屏幕,
用户看到的是「点了没反应」。弹窗要点一下才消失,跑不掉。

## 「筛选了不刷新」两端同病不同因(2026-09-12)

用户:「移动端媒体库页的筛选不生效,筛选了不会刷新出现筛选结果,PC 端也是」。
两端各有一道**为别的事装的闸**,顺手把换筛选一起挡了。

**安卓**:`LaunchedEffect(viewId, sort, minRating, genre)` 里第一句是
「已经有结果就别重拉」。而结果存在 `keepState("lib.<viewId>.items")` 里,
**这个键不含筛选条件** —— 换筛选重进这里时手里当然「已经有结果」,当场 return。
原注释还写着「筛选变了会带着新的 key 重进这里」,那句话是错的。
判据得是「手里这份**是按哪套筛选**拉的」(`needRefetch`,已提成可测函数)。

**桌面**:`Requery()` 清空网格后调 `LoadMore()`,而 `LoadMore` 第一行是
`if (_busy) return`。进库那一次、滚到底那一次都会把 `_busy` 举着 ——
于是换筛选**整个被吞掉**。光松开 `_busy` 还不够:旧那一次回来照样
`_grid.Append` 旧筛选的一页。要的是一个代次 `_gen`,换筛选加一,
在途的回来发现代次变了就把结果丢掉(三处出口都要判:结果落地、UI Post、异常分支)。

> 通例:凡是「避免重复请求」的闩,都要问一句**它凭什么认为这两次是同一次**。
> 判据里少一个维度,那个维度上的变更就会被静默吞掉,而且不报错。

## 媒体库默认排序 = 更新时间(2026-09-12)

用户:「媒体库筛选类型增加 更新时间 排序,默认从新到旧排序」。
`DateLastContentAdded`(这部剧最近一集入库)两端**早就在表里**,只是排在
`DateCreated` 后面而且没人点得动(见上一条)。默认档改成它:追更的人进库要看
「哪部剧刚更新」,而「加入时间」排出来的是「哪部剧刚被收进来」——
老剧更了新集永远排不上去。

## 收藏排序在核心层本地做(2026-09-12)

用户:「增加 收藏的排序功能,方便及时查看更新内容」。

- **不能交给服务端。** 某 fork 在 `Filters=IsFavorite` 上直接无视 SortBy 且照样回 200
  —— 送上去等于什么都没做(这条 2026-08 就记过,这次是它的第二次用武之地)。
- **排在核心层不排在两端。** 两端各写一份,改一处漏一处,而漏掉的那端只是顺序
  悄悄不一样,没人会报上来。命令 `emby.listFavorites` 收一个 `sort`,
  档位表是 `emby.FavoriteSorts`,第一条就是默认档。
- 默认给「更新时间」不是「加入时间」:收藏是用来追更的。
- 缺值一律沉底 —— 排在前面的话首屏全是「问不出这个值」的条目,看着像列表坏了。
- 四档的测试期望顺序**两两不同**。写成同一个顺序的话,四档塌成一档也照样绿。

## 标签与工作室:按名字筛是假的,按 id 才真(2026-09-12)

用户要「增加 标签 工作室,同时支持点击 标签 工作室 类型 的跳转」。
拿真服(Emby 4.9.5)实测了一遍,四条都是跑出来的:

| 查法 | 结果 |
|---|---|
| `Genres=纪录` | **生效**,28 条全对(注意:某 fork 上是假的,老账仍然成立) |
| `Studios=<名字>` | **完全无视** —— 返回全库 1673 条,头几条的工作室对不上 |
| `StudioIds=49567` | **精确命中 1 条** |
| `StudioIds=a,b` / `a|b` | 两种分隔符都认,返回并集 |

所以点工作室 chip 走的是 `studio_ids`,不是 `studios`。写成名字的表现是
「点了工作室,出来的是一整个库」,**一句错都不报**。

**数据形状也得实测**:
- `Studios` 是**对象数组** `[{"Name":"…","Id":49567}]`,而且 **Id 是数字**;
  Jellyfin 那边是 GUID 字符串。声明成任何一种都会在另一家上解析失败,
  而失败的表现是**整个条目解析报错**,不是少一个字段。所以收成 RawMessage 再拉平成字符串。
- `Tags` 在那台真服上**全库一条都没有**(60 条抽样 0 命中)。所以前端没值要整行不画,
  别摆一个永远空的「标签」区。
- `/Items/Filters` 和 `/Items/Filters2` 在那台上都是 **404** —— 分面那条路本来就要能容忍。

☠ `ItemQuery` 里 `Tags`/`Studios` 原来**只发出去、不复筛**,而 `needsLocalFilter`
又把它们算作「要复筛」。于是在无视这两个参数的服务器上,复筛一遍什么都没滤掉,
整页不匹配的条目照样铺出来。已补进 `matches()`。

> 通例:凡是「发出去的筛选参数」,都要问一句**服务端要是当空气我会怎么样**。
> 答不上来就说明还差一道客户端复筛。

## 闸门的窗口只有 30 行(2026-09-12)

`check-android-fields.py` 只在「调用点往下 30 行 / 下一个 fun / 下一个调用点」这个窗口里
找取值器。详情页那种「开头拉一次、几百行之后才画」的页面,后面那些读取**它照不到** ——
实测把 `namedList("studios")` 改成 `namedList("studioz")`,闸门照样打 ✓。

顺手把取值器表补全了(`namedList` / `boolOrNull` / `arr` / `obj` 原先都不在表里),
但**窗口这条限制仍在**。别把「闸门绿」当成「字段名对得上」。

## 「测试连接」探测失败,不等于这台服务器不能用(2026-09-16)

接上面「只探公开信息等于没测」那条 —— 那次改成「给了账号就真登一次」是对的,
但探测那一步仍然是**硬前置**:`ProbeServer` 一挂就 `return`,根本走不到登录。

用户报:「测试链接遇到 UA 白名单的服务器直接报错了,**直接登录倒是没有问题**」。
两句话自相矛盾,而用户只会认为测试连接是坏的。

根因:`/System/Info/Public` 是**匿名端点**,`/Users/AuthenticateByName` 是**带凭据端点**,
它们在反代 / WAF 眼里是两条路 —— UA 白名单、只放行带凭据的路径、
干脆把公开信息端点关掉的都有。拿匿名端点的结果去替带凭据端点签字,本来就不成立。

改法:填了账号时**登录才是判据**,登得进去就算通;服务器名 / 版本拿不到就留空。
两条都不通时,报错里要把探测那句也带上 —— 只说「登录失败」会让人一直去改密码。

> ★ 用例 `TestTestConnection探测被拦但登录得进去也算通` 反向注入验过:
> 改回「probeErr != nil 就 return」当场红在 `E_NETWORK … HTTP 403`。

## 屏蔽名单迁移时漏了落盘(2026-09-16)

`core/blocklist` 在 Go 版里只有一个**包级切片**:`Set()` 写完就完了,`Replace()` 在整个
`core` 里**没有任何生产调用者**(穷举 grep 只有 diffcheck 和测试)。
也就是说屏蔽名单**重启就空**,而且一声不吭。Rust 版一直是落 `blocklist.json` 的。

它一直没被发现,是因为条目级屏蔽的入口被 `Features` 关着 —— 功能没人用,
自然没人发现它不持久。这次要做「屏蔽媒体库 → 卡片变灰 → 右键恢复」才撞上:
灰卡下次开机自己变回来,用户只会认为恢复键是坏的。

修法:`Load()` 在 `lp_init` 里 `paths.SetRoot` 之后、命令注册之前调一次;
`Set()` 自己落盘(原子写:临时文件 + rename,和配置同一个口径)。

> ★ 用例 `Test屏蔽名单要落盘` 反向注入验过:拿掉 `save(snap)` 当场红在
> 「重启后名单对不上: []」。**要害是用例里那句 `Replace(nil)`** ——
> 不把内存名单清掉,怎么读都是对的,那就是条恒绿的假门禁。
>
> 通例:换栈时**「有没有落盘」这件事编译器和单测都不会提醒你**。
> 凡是 Rust 版有 `xxx.json` 的,迁移后要逐个去数据目录里确认文件真的出现了。

## 聚合视界「聚合不到」:按 TMDB id 补,不是拉全库(2026-09-17)

用户:「一些新后端的 Emby 接口有问题,使用聚合视界的时候会聚合不到别的剧集」。
两台真服(fork 4.9.3.0 / 4.10.0.40)实测的服务端搜索行为:

| 形态 | fork | 4.10 |
|---|---|---|
| `Items?SearchTerm=` | 子串(「时代」搜得到「小时代1：折纸时代」) | 按**词前缀**(「V字仇杀队」搜「仇杀队」= 0);同 PresentationUniqueKey 的几个译名只回一条,回哪条随查询变 |
| `AnyProviderIdEquals=tmdb.<id>` | 真过滤(不存在的 id → 0) | 真过滤;逗号分隔 20 个 id 一次 2.2s |
| `/Search/Hints` | 未测 | 17 个查询全回 `[]` |
| 分页 | **每页硬夹 200,不认 Limit,也不认 Fields** | 不夹 Limit |

两台共有 3718 部(按 TMDB),其中 **151 部名字不一样**。抽 40 部,拿一台的名字去另一台服务端搜:**命中 3**。
这就是「聚合不到」—— 不是接口坏了,是译名不同。

**做法**:每台服先用自己的 SearchTerm 搜;把所有命中的 TMDB id(按类型,电影和剧是两套编号)
拿去每台服 `AnyProviderIdEquals` 查一次补齐(`emby.ByTmdb`)。同样 40 部两个方向都是 **40/40**,
聚合搜索中位 1.5s、最慢 2.6s。跨服版本:电影按 TMDB 查到唯一一条就直接认(不再要求标题相同),
剧按剧 TMDB 找剧再按季集号取集(翻页,不再只看第一页 200 集),查不到才退回名字搜索。
实测电影 15/15、分集 11/12 认对,认错 0(没认出的那集是对面库里只有第 3 季)。

- ☠ `ByTmdb` 回来的结果**本地再按 TMDB 滤一遍**:不认这个参数的服会回整个库,不滤就是那台服全部片子都成了「同一部」。
- 词前缀那台搜不到子串没关系:只要另一台搜得到,TMDB 会把它带过来。两台都搜不到的才真搜不到。
- 每台服 20 秒超时;没搜成的服带 `error`(桌面以前不读,全失败显示成「没搜到」);
  聚合视界总览连不上的服带 `error`;TV 的跨服版本卡不再自己按名字判,改调 `emby.aggregateVersions`,
  `confidence=possible` 标「可能匹配」。

**走过的弯路(别再走)**:第一版把每台服的电影 + 剧目录整个拉到本地做子串匹配。命中率一样是 40/40,
但 fork 那台 2.9 万条、每页 200 条,串行 101.6s、6 路并发也要 17s,首次搜索要等,还常驻内存和磁盘。
用户一句「Emby 不是有专门的搜索接口吗,没必要拉完」点醒:缺的只是跨译名对齐,TMDB id 查询一次就够。
