...
## 只查不装的「检查更新」等于没有(2026-09-11)

用户原话:「不然检查更新也没啥用,直接软件内一条龙更新安装就行了」。
当时的实现查到新版就吐一条 URL 完事,而绿色包的手动更新恰恰是最容易丢数据的一步。
连同这次一起挖出来三个**互相独立、全都不报错**的断口:

| 断口 | 表现 | 真因 |
|---|---|---|
| 安卓永远说「已是最新」 | 有新版也不提示 | `checkUpdate` 的版本号在 `update` 子对象里,UI 直接 `.str("version")` 恒为 null |
| 安卓永远挑不出安装包 | 就算查到了也只能跳网页 | `assetKeywords()` 只分 windows / linux,GOOS=android 落进 linux 分支,而 APK 名里没有 `linux` 这三个字母 |
| 「启动时检查更新」是个死勾选框 | 勾了不勾一个样 | `update_auto_check` 从落库那天起**没有任何人读过**(`grep -rn auto_check core/ apps/` 只有读写没有调用点) |

做「装」这一半时踩到的几条:

- ☠ **覆盖只加不删。** 绿色包的用户数据就在 exe 同级的 `userdata/` 里。
  robocopy 一旦带上 `/MIR`,它会把「源里没有的」当成多余文件删掉 —— 连账号一起。
  这一步跑的时候程序已经退出了,炸了连日志都没人写,所以做成了单测钉死的约定。
- ☠ **必须钻进 zip 里那层单独的顶层目录。** `pack-win.sh` 用 `Compress-Archive`
  打的是整个文件夹,真东西在 `LinPlayer/` 下面。照解压目录直接覆盖的结果是安装目录里
  多出一个 `LinPlayer/LinPlayer.exe`,旧 exe 原地不动 —— **更新看着成功,其实一次都没换**。
- ☠ **下载地址不收 UI 传的 URL。** 装什么只能由核心层自己从发布接口解析出来,
  否则这条命令就是一个「下载任意文件并以本机权限执行」的入口。
  同理压缩包里的路径是外来输入,`../` 要挡(zip slip)。
- ☠ **长度对不上就判失败。** 半个 zip 多半还能解出前半截文件,那是最糟的形态:
  安装目录被半份新版覆盖,而用户手上只剩一个起不来的目录。
- ☠ **覆盖脚本得等本进程退出。** exe 正被自己锁着。Windows 上用
  `Wait-Process -Id <pid>` 而不是 `tasklist | findstr` —— 后者的输出是本地化的,
  中文系统上匹配不到。脚本起完就撒手,不 Wait(等它等于等自己退出)。
- ☠ **生成的 `.ps1` 必须写 UTF-8 BOM。** 路径里只要有一个中文字符,
  没有 BOM 的脚本会被按 GBK 读、字节错位,而 PowerShell 不报错。
  和 `pack-portable.ps1` 当年整个打不出包却 CI 全绿是同一个坑。
- 安卓那边 `REQUEST_INSTALL_PACKAGES` 只是「允许申请」:用户还得在系统设置里
  开「安装未知应用」。不查 `canRequestPackageInstalls()` 就发意图的表现是
  **什么都不发生** —— 正是最难查的那一类。所以先问,没开就把人送到那一页去。

## 断言跟着实现返回的路径走 = 假绿(2026-09-11)

下载那条测试第一版是这么写的:拿命令返回的 `file` 去 `os.Stat`,再看 `file + ".part"`
在不在。注入「`.part` 根本不改名,直接返回它」—— **测试照样绿**:
返回的路径确实存在、大小也对,而 `x.zip.part.part` 本来就不该存在。

断言不能顺着被测代码给的那根线走。改成按**目录里的事实**判:
最终文件名必须等于资产名,而且那个目录里一个 `.part` 都不许剩。注入当场变红。

## 固定文案的「更新说明」比空白更糟(2026-09-12)

用户:「优化更新 md,现在一直是固定的,牛头不对马嘴」。查下来是**两头都坏**:

| 渠道 | 说明从哪来 | 症状 |
|---|---|---|
| 预览版(`build.yml` 建的 `-pre`) | 压根没写 `body` | 应用内「更新说明」永远一片空白 |
| 正式版(`publish.yml`) | workflow 里写死的一段话 | 每次发布一字不变,内容是「下载在哪、数据目录在哪」 |

固定文案最坏的地方不是没用,是它**看起来像更新说明** —— 用户读完会以为这次
就改了这些。而应用内那个弹窗还把它截到 600 字,固定文案正好占满,
真正改了什么被切在后面。

改法三处,缺一条都不够:

1. `scripts/release-notes.sh` 从提交历史生成。基准取上一个**正式版** tag ——
   预发布 tag 在提升为正式版时会被删掉(`publish.yml` 最后一步 `--cleanup-tag`),
   拿它当基准会漏掉整整一个版本。`类型(范围): 说明` 转成 `【范围】说明`:
   范围就是「改的是哪一块」,而 feat/fix 对用户没有意义。
2. `publish.yml` 把预发布的说明**原样带过来**(那就是这一版真实的提交清单),
   下载指引挪到它**后面**。这个 job 没有 checkout 仓库,在这儿重算是算不出来的。
3. 应用内不再截断:`Updater.Notes()` 换成一个封顶 360 高的 `ScrollViewer`。
   安卓端更是**第一次**把说明显示出来 —— 之前点一下就直接开下载,
   既看不到改了什么,也没问过用户(手机上多半是流量)。

## 「默认开」的开关改成「默认关」时,光改默认值没用(2026-09-12)

用户:「自动检查更新默认关,需用户打开自动更新再自动检查更新」。
`DefaultPrefs()` 里把 `UpdateAutoCheck` 改成 false 是不够的:偏好是**整体落盘**的,
凡是动过任何一项设置的用户,盘上都明明白白写着 `"update_auto_check": true`,
解析时它会盖掉新默认值 —— 改完对老用户一点效果都没有,而且不报错。

这一处的解法是**换 JSON 键名**(`update_auto_check` → `update_auto_check_optin`)。
能这么干是因为这个开关从落库到 2026-09-11 之前**一个消费者都没有**:
盘上那个 true 不是任何人的选择,只是当年的默认值被整体落了盘。旧键留在配置
文件里没人读,是死数据。测试两头都钉:老键不许被读进来、新键必须读得进来。

> 换键名只在「老值不代表任何人的意图」时才成立。若用户真的选过,
> 要么留一次性迁移,要么就别动 —— 悄悄改掉用户设过的开关比默认值不对更糟。

## 「最新版还提示更新」是**显示**把区分位抹掉了(2026-09-12)

用户:「处于最新版但是依然提示更新,姑且点更新,更新完启动再次提示更新」。

拿真实发布数据跑了一遍 `CheckUpdate`,比较逻辑**是对的**:

```
stable      cur=1.1.0-build730 -> 已是最新
prerelease  cur=1.1.0-build733 -> 已是最新
prerelease  cur=1.1.0-build730 -> 提示更新到 v1.1.0-build733-pre
```

真正的病灶在界面:`Info.Version` 过了一道 `NormalizeVersion` 规约成 `x.y.z`。
而预发布渠道**一天出好几个 build** —— 实测 2026-09-12 那天 build731 / 732 / 733
前后隔 12 分钟。规约完三个版本在弹窗上长得一模一样,都叫「新版本 1.1.0」,
用户手里那个也叫 1.1.0。于是「明明最新还在提示」,更新完再弹一次也分不出新在哪。

改成 `DisplayVersion`:只去掉 tag 前面那个 `v`,`-buildN` / `-pre` 原样留着。
「已经是最新版本了」那句也要**报全版本号** —— 下次它又弹更新时,用户得有东西可以对照。

> 通例:凡是「给用户看的版本串」,规约之前先问一句**它还分得出两个相邻版本吗**。
> 分不出的话,所有基于它的判断在用户眼里都是随机的。

## 覆盖安装到底落在哪儿:真跑一遍才算数(2026-09-12)

用户报「自动更新完之后,用户自己创建的快捷方式不能使用了」。原先这一段的测试
**全是对脚本字符串做断言**,一次都没真跑过 —— 而「落到了哪儿」正是字符串照不到的。

补了 `Test覆盖脚本真跑一遍就地换文件`:造一个中文路径的安装目录,真跑一遍
PowerShell,回来数文件。结论是覆盖这一步**没问题**:

- exe 是**同一个路径换内容**(所以 `.lnk` 的目标没动过)
- 安装目录里不会多出一层 `LinPlayer/`
- 子目录里的文件会换,新增的文件会带过来
- `userdata/` 原样活着(robocopy 不带 `/MIR` 的全部理由;注入 `/MIR` 这条测试当场红)
- 暂存目录清干净

★ 暂存目录**必须摆在安装目录里面**再测:真实布局是
`<安装目录>/userdata/cache/update/staged/LinPlayer`,「源在目标里面」这个几何
摆到外面去测根本测不到。第一版就摆错了。

所以「快捷方式失效」的成因不在覆盖这一步,还没定位到。

## 更新说明从「上一个版本」列起,不是从上一个正式版(2026-09-13)

用户:「为什么更新日志一直对比 730 版本,应该是对应上一个版本的」。

`scripts/release-notes.sh` 原来只认上一个**正式版** tag 当起点,而正式版很少发 ——
build731~742 每一个预发布都从 730 列起,清单越攒越长(742 那版列了 20 条,
其实它只比 741 多 1 个提交)。

现在起点是 `git describe --tags --abbrev=0 --match 'v*' HEAD~1`:上一个版本,预发布正式版都算。
按提交图找最近的祖先 tag,不按 tag 名字排序;从 HEAD~1 找是因为重跑时这一版自己的 tag
可能已经打上了。结尾那行「共 N 次提交,对比 X」也删了 —— 用户:「做的复杂还搞对比着实没必要」。

☠ 上线后第一版(build753)还是多列了一条:`build752-pre` 的 tag 落在了 **753 的提交**上。
  softprops/action-gh-release 新建 tag 时默认打在**默认分支最新**,而不是这次构建的提交 ——
  两次推送挨得近时,前一个 run 发布的那一刻 main 已经前进了。于是 753 从 HEAD~1 往回
  找不到 752,一路退到 751。build.yml 的 Create Pre-release 加 `target_commitish: ${{ github.sha }}` 钉住。
  (752 那个 tag 已经打错了,不去改历史,之后的版本是对的。)
...
## 又一个「本机全绿、CI 红」:字形只在 Linux 打包时查(2026-09-21)

给侧栏的插件入口随手挑了个图标字形 `U+E74C`,本机六关全过、桌面编译通过、推上去
**Linux 打包当场红**:

```
U+E74C 在界面里用了,LinIcons 里没有 —— Linux 上是豆腐块
```

根因不是那个字形有多特殊,是**这一关只写在 `pack-linux.sh` 里**。Windows 上认不得的
字形会被系统字体替补,所以本机看不出来;Linux 没有那个替补,就是一个方块。

修法两条,缺一不可:
1. 用已经在 MAP 里的字形(`E71D = apps`,本来就是「插件」那个图标);
2. 把 `gen-icon-font.py --check` **搬进 `check-style.sh`**(第 7 关)——
   推之前本机就能红。反向注入验过。

这是 build-release 里记的第五种 CI-only red 的又一例:**判据只挂在某一个平台的打包脚本上**。
判断一关该放哪儿的问题很简单:「不装那个平台的人改了这块代码,他本机会红吗?」
答案是否,那这一关就该往前挪。
