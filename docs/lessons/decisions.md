...
## 图标源:内置的只报条数,不报地址(2026-09-12)

用户要「允许用户自己添加网络源」。做法是 `Prefs.IconSourcesExtra` 存用户加的那几条,
和编译期注入的内置源**并存**,`IconSources()` 合并去重。

- **只存用户加的。** 把内置的也存进盘,编译期换了源之后,老用户盘上那份旧地址
  会一直盖着它,而且没人会想起来去清。
- **去重按规整后的串比**(去掉结尾斜杠)。用户看得见内置源的图标,很可能把同一条
  地址也手抄一遍 —— 不去重的话同一批图标在网格里出现两遍,没人看得出为什么。
- **界面上内置源只报条数,不报地址。** 地址走编译期注入(仓库红线:域名不进提交),
  摆到界面上等于把它抄进了用户的截图和日志里。
- 换了源要**顺手清缓存**:不清的话加完源点刷新,回来的还是 24 小时前那份。
