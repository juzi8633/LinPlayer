...
## 「旁边的窗口」不等于再开一扇顶层窗口(2026-09-09)

弹幕搜索面板做成了**贴在播放页右侧的一块 Border**,不是一个新的 `Window`。

理由:这一页所有浮层能稳稳压在画面上,靠的是「它们都住在同一个透明的 Avalonia 窗口里」
—— 底下那个 mpv 视频窗是**另一个顶层窗口**。再开一扇窗就要重新解决一遍
z 序、窗口跟随、DPI、Alt-Tab(`win-wndproc-hook-reentrancy` 和
`fullscreen-white-edge-geometry` 两篇记的都是这一类账),而收益只是同一块内容。

同一份内容在手机上是**居中模态弹窗**:那边的面板只有 236dp 宽,
而搜索结果是「源 → 作品 → 集」三层,一列摊不开。版式按屏幕的形状来,不强求一致。

进度条的弹幕热力图画在**轨道上方那 9 像素**里,不叠进轨道:
叠进去的话它和「已缓冲」那条半透明带子互相染色,两个都读不出来。
`PlayerBar` 的热区高 24、轨道居中在 10..14,上面那一截本来就是空的。

---

## 窗口要能拉到任意大小,`MinWidth` 只是第一层(2026-09-11)

`MinWidth=1024` 只是把问题挡在门外。放开之后真正会坏的是三类地方,
它们**都不报错**:

1. **固定宽的侧栏。** 212px 在 900 以下就占掉四分之一 ——
   低于 900 自动折叠成 72,拉宽再自己回来。★ 用户手点过的收放优先:
   自动收了要记一个 `_autoCollapsed`,否则「我明明展开了它又自己关上」。
2. **`Auto,*,Auto` 的两头。** 两个 Auto 列加起来超过窗口宽时 Grid **不报错,
   只是画在一起** —— 看上去是「按钮糊成一团」。播放页底栏就是这样。
   收的顺序按「丢了还有别的办法」排:音量有滚轮和快捷键、±10 秒有方向键;
   播放/暂停、时间、全屏、字幕一个都不收。
3. **横排的筛选条。** `StackPanel` 放不下会把最后一个**直接切掉**,
   一点提示都没有。换 `WrapPanel` 至少还看得见。

水槽(18)是按 1280 宽定的比例,窄窗口收到 10 —— 挂在 `PageBase.Scrolled`
一处,各页自己写就会漏。

## 长标题挤走功能键:跑马灯要占**弹性列**(2026-09-11)

播放页顶栏原来是 `back + 标题(MaxWidth=620,省略号)` 一起待在 Auto 列里。
两个毛病:窗口一窄右边那排功能键被推出画面;省略号意味着长片名后半截
**永远看不到**。

正解是把标题放进 `*` 列,放不下的部分交给跑马灯(`Views/Marquee.cs`)。
三条和安卓那条跑马灯共用的口径:

- **速度写成像素/秒(30),不写死总时长。** 写死的话标题越长划得越快,
  而越长的标题恰恰越需要时间读。
- **放得下就彻底停下来**,不空转一个 60Hz 的定时器。
- **滚到尾停一下再跳回开头,不来回弹。** 来回走眼睛得跟着换方向,更难读。

## 集详情页的剧名要点得动(2026-09-11)

Emby 上点集详情页的剧名就回到剧集主页。我们把剧名和集名拼成一句
(「剧名 · 第 3 集」),那一整句都是死的 —— 从某一集想回到整部剧只能一路按返回。

★ 判据是拿不拿得到 `series_id`:刮削不全的库上这个字段是空的,那时候照旧
只显示文字。摆一个点了没反应的链接比没有更糟。

---

## 「能自由缩放」不等于「适配了」:页面里的写死尺寸才是主体(2026-09-11)

放开 `MinWidth` 只是让窗口拉得动。用户当天就打回来:「首页、剧详情页、集/电影详情页、
媒体库详情页这些都要适配呀,不是说简单的允许自由缩放就行了,里面的样式也要能够自由缩放」。

实际数出来的写死尺寸(全是按 1280 窗口定的):

| 位置 | 写死的数 | 窄窗口上的样子 |
|---|---|---|
| `MediaGrid.MinCardWidth` | 158 / 256 / 库卡 320 | 内容区 300 宽时**一行只剩一张巨卡** —— 网格等于失效 |
| `HomePage.Strip` | 轨道卡宽 158 / 256 | 一条轨道只看得见一张半,而横向轨道的全部意义是一眼看到一排 |
| `DetailPage` 头部 | 海报 220×330 / 剧照 392×220 | 海报自己吃掉整条宽度,右边的标题/小片/简介/播放键**整片挤出画面** |
| `DetailPage` 标题 | 34px | 占掉三行 |
| 分集 / 合集 / 演职人员轨道 | 214 / 168 / 头像 84 | 同上 |
| `Carousel` 翻页键 | 40px 直径 ×2 | 加起来把一整张卡盖掉 |
| `Hero` | 标题格 92、标题 36px、高度下限 280 | 360 宽的窗口上头图占掉 78% 的高度,首页只剩头图 |
| 媒体库详情页筛选条 | 150+150+120 | `StackPanel` 放不下就**直接切掉**最后一个,零提示 |

统一成 `Views/Responsive.cs` 一处:`内容宽 / 1030` → 系数。四条约定都踩过坑:

- **只往小缩,不往大放。** 往大放会改掉用户已经认可的那一档版式,而他报的是变小之后的事。
  副作用是 1280 窗口上**一个像素都没变** —— 这正是要的。
- **系数量化到 0.05 一档。** 不量化的话 `SizeChanged` 每帧都是一个新系数,
  凡是靠它重建控件的地方(横向轨道)就变成每帧重建一屏卡片。量化后整段拖动最多 9 档。
- ☠ **先量化再夹,不能先夹再量化。** 下限 0.62 量化成 0.60,反而掉到下限以下 ——
  自检第一次运行就是这么红的。下限必须落在量化网格上(0.05 的整数倍)。
- **字号不能和尺寸共用系数。** 卡片缩到 0.60 还能看,12.5px 的字缩到 7.5px 就读不了了。
  字号走「缩一半幅度」:`1 − (1−s)/2`。

比缩放更关键的一条:**窄到一定程度要改排法,不只是改尺寸。** 详情页头部在内容宽
低于 620 时从左右两列改成上下两段 —— 海报和右列本来就在抢同一条宽度,
再怎么等比缩都是两个都太小。

## 壳没有单测工程,那就做成能跑的探针(2026-09-11)

`apps/windows` 一个测试工程都没有,而缩放曲线是纯算术:改坏了编译全绿,
只会在真机上表现成「窗口缩了里面没缩」。照着既有的 `LP_FONTPROBE` /
`LP_SCROLLPROBE` 加了第三个:

```
LP_SCALEPROBE=1 LinPlayer.exe     # 打七行就退,不开窗口,失败条数进 ExitCode
```

它**第一次运行就抓到一个真 bug**(上面那条先夹后量化)。反向注入四处各自变红。

两条做这种探针时踩到的坑:

- **注入脚本的锚点必须唯一。** 第一版拿「那一行公式」当锚点,还原时按首次出现替换 ——
  把 `S` 和 `Font` 两个公式**互换**了,而两条断言照样全绿。现在 `patch` 里带
  `assert count == 1`。
- ☠ **断言别让 `min` 把自己夹成假绿。** `Font(300, 34, 21) > 34 * Of(300)` 看着在比曲线,
  实际结果被下限 21 夹住,把公式换成和尺寸同一个系数照样过。
  改成直接比曲线本身(`HalfOf(300) > Of(300) + 0.05`),注入才红。
  同理「300 宽还能并两张卡」测的是上界,**测不到下限** —— 下限要单独断言。

## 只测驱动器,测不出「翻页按钮卡死」(2026-09-12)

2026-09-08 已经修过一轮「点左右翻页按钮有概率卡死」,还配了 `LP_SCROLLPROBE`。
用户 09-12 再报同一句话。回头看,那一轮修的和测的都只是**驱动器本身**,
而真正漏掉的那一处在**入口**:

```csharp
// Smooth.GlideX / Smooth.Wheel
if (!d.Running) { d.TargetX = sv.Offset.X; ... }   // ← 判的是字段
```

`Running` 在**帧不再来**的时候会永远停在 true(窗口最小化、页面被顶掉,
`RequestAnimationFrame` 挂在渲染循环上,排进去的那一帧永远不回调)。
`Run()` 里已经用 `StillAlive(Running, LastFrame)` 认这种「挂着但死了」,
**入口这两处却没有** —— 于是目标一直叠在一个永远到不了的旧值上,
表现正是「按钮还亮着,点下去一动不动」。两处都改成判 `StillAlive`。

配套加了 `LP_RAILPROBE`:一千条数据 → 真 `Carousel.Rail`(虚拟化面板 + 两颗真按钮)
→ 从头点到尾再点回来。它一上来就抓到自己的假绿:

☠ **没有可视根时 `ScrollViewer.Extent` 恒为 0。** `LP_SCROLLPROBE` 正是靠这个形状
去逼死角的,但拿它测「能不能滚到尽头」时,Extent=0 意味着「滚哪儿都一样」——
四条断言全部白白变绿。所以轨道探针**必须开一个真窗口**(`w.Show()` + 手动泵
`Dispatcher.UIThread.RunJobs()`),代价是它不能进 CI,和 selfcheck 一起手跑。
三个探针现在有一条命令:`bash scripts/probes-win.sh`。

## 长按左键拖轨道:取消卡片按下态的正路是 capture-lost(2026-09-12)

用户 2026-09-12:「增加长按鼠标左键向左滑动,反之向右滑动」。
轨道里每张卡自己就是个 `Button`,所以拖拽的难点全在「别让这一下变成点击」。
两条看着都对的写法,实测(合成指针事件跑 `LP_RAILPROBE`)都咬人:

| 写法 | 症状 |
|---|---|
| 起拖时 `e.Pointer.Capture(sv)` 抢指针 | **没用**。Avalonia 的 `Button` 按下时并不抓指针,抢了等于没抢,松手照样跳进详情页 |
| 松手那一下 `e.Handled = true` 吃掉 | 拖拽不再误点了,但 `Button.IsPressed` **永远停在 true**,它的状态机没走完 —— **下一次点卡片被整个吞掉** |

正路是在越过阈值那一刻**给按下的那个控件补发一次 `PointerCaptureLost`**:
`Button.OnPointerCaptureLost` 就是它自己的取消入口,发完 `IsPressed` 归位,
松手不会点,而下一次点击照常。

`LP_RAILPROBE` 里这几条是这么钉住的(合成 `PointerPressedEventArgs` 等,
事件**从卡片自己发出去** —— 这样隧道阶段经过轨道、冒泡阶段回到卡片,
和真鼠标同一条路由;直接在 ScrollViewer 上 raise 的话卡片根本收不到):

- 拖 160px 走 160px、松手 `clicks == 0`
- **拖完之后再点一下,`clicks == 1`**(上面那条「吞掉下一次点击」就是它抓的)
- 抖 5px 仍然算点击、越过 6px 转成拖拽

起拖阈值 6px 不能省:手按在卡上点一下,指针总会抖一两个像素,不设阈值的话
每一次点击都被判成拖拽,卡片就再也点不开了。

## 窗口大小要自己记(2026-09-12)

用户:「窗口大小无法记忆,一直缩放到最小,第二次打开也不会按照尺寸自动缩放」。
尺寸原来写死在 `MainWindow.axaml` 的 `Width="1280" Height="800"` 上,
没有任何地方读过或存过。现在落在核心层偏好 `window_w / window_h / window_max`,
`Views/WindowMemory.cs` 负责起手恢复(必须在窗口显示**之前**,晚一步用户
会看见它先弹出默认大小再跳一下)和防抖落库(700ms;拖窗口时 SizeChanged 每帧都发)。

三条不能省的判断:

- **只记普通窗口态的尺寸。** 最大化 / 全屏时量到的是整块屏幕,记下来等于
  「退出最大化之后窗口和屏幕一样大」;最小化时某些窗口管理器报 0。
- **恢复时按当前屏幕的可用区域夹一下。** 换显示器 / 拔外接屏之后,
  存下来的尺寸可能比现在这块屏还大,窗口有一半在屏幕外 —— 而标题栏
  可能正好落在看不见的那一半里。
- 「换页面之后窗口变小了」是**另一条**:退全屏一律回 `WindowState.Normal`
  (那是给页面内按 F 退全屏设计的),而**离开播放页**也走那一条,
  于是最大化看片、回来变成小窗口。改成 `SetImmersive(true)` 时记下进来前的状态,
  `SetImmersive(false)` 时还回去 —— 不动 `SetFullscreen` 的语义。

## 打了字没人听:探针不置退出码(2026-09-12)

`LP_SCROLLPROBE` 从落地起就只 `Console.WriteLine("PROBE 滚动 ✗ …")`,
**一次都没设过 `Environment.ExitCode`**。而 `scripts/probes-win.sh` 是按退出码判的
—— 这一组探针不管红成什么样,脚本都报「探针全部通过」。

这比没有探针更糟:它在报告里占着一行绿。加探针时**先想清楚谁在读它**,
写文件、打屏、置退出码是三件不同的事。

## 顶栏的网速读数(2026-09-12)

用户:「网速显示要常驻,移动端和桌面端都是」。桌面端此前**一个字都没有**。

口径和安卓一致:**整机**(所有 `OperationalStatus.Up` 且非 Loopback 的网卡
`GetIPStatistics().BytesReceived` 求和)。用户要问的是「现在到底还有没有在下东西」,
而播放中封面、弹幕、上报都在跑,只报这一条流答不了那个问题。

三条:

- 采样搭在**已有的 `_poll`(250ms)**上,自己看表够一秒才出一个数 —— 不另起定时器。
- 取不到统计返回 **-1 不是 0**:0 是合法读数,拿它当「没数」会让那一格永远显示 0 KB/s。
- 读数放在 `_top` 里,显隐自然跟着 OSD 走;**采样不跟** —— 跟了就是安卓那个坑
  (见 `docs/lessons/android.md` 同日一条)。

## 轨道到底之后差一个间距(2026-09-12)

用户:「划到底之后有一个弹动然后自动把最后一集贴边即可」。
「贴不上边」是真的:虚拟化面板没有 `Spacing`,间距只能加在**每一项自己**身上,
于是**最后一项后面也挂着一个 `RailGap`** —— 滑到底右边空着 16px,
看着像「还没到底但不动了」。

改法是给 `ItemsControl` 一个 `Margin = (0,0,-gap,0)`:负外边距把量程收回来。
逐项判「是不是最后一个」这条路走不通 —— 虚拟化面板手里没有全表,
`FuncDataTemplate` 也拿不到序号。

探针先红后绿:卡右沿 884 → 900(视口 900),量程 46000 → 45984。

**到边回弹弹的是 `RenderTransform` 不是 `Offset`** —— ScrollViewer 自己会把越界的
Offset 夹回去,拿 Offset 根本弹不出去。触发点两处:翻页键要的比剩下的多
(`GlideX` 里记一笔,滑完那一刻兑现)、拖拽越界超过 8px 松手。

## 探针里插新段落要管好起点(2026-09-12)

在轨道探针中间插了一段「滑到底看贴边」,把它后面那条
「上一轮还挂着但帧不来了之后点一下还走得动」顶红了 —— 那条是从**尽头**开始点「›」,
挪不动是应该的。红的是探针不是代码。插段落之后要把位置复位。

## PC 播放页的锁定键(2026-09-12)

用户:「PC端增加锁定按钮在左侧中间,方便用户沉浸式观看」。

- **只锁 OSD,不锁键盘。** 锁掉快捷键的话连暂停和音量都没了,那不是沉浸是瘫痪。
- 锁上之后上下两条再也不自己冒出来,**但那颗锁自己仍然跟着 OSD 显隐** ——
  不然锁上之后没有任何东西能解锁。
- 锁定键改的是 `_locked` 而不是 `_osdOn`,走 `ShowOsd` 会被开头那句
  `if (_osdOn == on) return;` 挡掉、点了没反应。所以把兑现那一段抽成 `ApplyOsd()`,
  两个入口共用。

## GitHub 代理(2026-09-12)

用户给了三档现成的,并要求「支持用户自定义」。三条:

- **查版本和下载两条都要走代理。** 只代理下载的话,墙内用户连「有没有新版」
  都问不出来;只代理查版本的话,查得到新版却下不动。
- **只套 GitHub 自己的域**(`api.github.com` / `github.com` / `objects.githubusercontent.com`)。
  套到假上游(`LP_UPDATE_API`)或自建镜像上,那条路当场打不通 ——
  而那正是测试和排障走的路。
- 界面给的是**输入框 + 几颗快填按钮**,不是下拉:这类公共代理今天能用明天就 404,
  只给下拉等于把人锁在坏掉的那几个上。档位表由核心层给
  (`prefs.getUpdateSettings` 的 `proxies`),两端不各抄一份。

## 一类问题一起出:全仓还有谁在用自定时器做动画(2026-09-12)

修完弹幕那条之后 `grep -rn DispatcherTimer` 扫了一遍桌面壳,又抓到一个:
`Marquee`(顶栏那条跑马灯标题)也是 16ms 自定时器。用户没报过它 ——
标题滚得慢,抽帧不明显,但那是同一个毛病。一并改成 `RequestAnimationFrame`。

剩下的 `DispatcherTimer` 都是**轮询 / 防抖 / 一次性延时**(250ms 播放轮询、
90ms 网格防抖、Toast 的 RunOnce),那些不是逐帧动画,用定时器是对的。

> 判据很简单:**这个定时器的每一拍会不会改一个屏幕上看得见的位置/透明度?**
> 会 → 该用渲染帧;不会 → 定时器合适。

## 分集卡那行小字:数据早就在回,只是没人画(2026-09-12)

桌面草稿 03 页第 16 条要求分集卡「除标题外加一行小字:分辨率 / 码率 / 大小」。
查下来**一行核心层代码都不用改**:

- `Client.SeasonEpisodes` / `episodes` 的 URL 里本来就带 `Fields=…,MediaSources`
- `core/emby.Item` 从落地起就有 `video_height` / `bitrate` / `size_bytes`
  (注释还写着「仅在请求带 Fields=MediaSources 时有值」)
- 桌面的 `CardItem` 三个字段一个都没接,卡上印的是时长

又一次「后端领先前端」。判断一个界面需求要不要动核心层之前,先 grep 一遍
`core/emby/emby.go` 的 struct —— 那张表比想象中全。

两条口径:

- **写法要比媒体信息那一块短**:`18.4G` 不是 `18.4 GB`。分集卡最窄 140px,
  带空格的写法会被省略号从后面吃掉,而被吃掉的恰恰是体积。
- **回落逻辑要放进可测的属性里**(`CardItem.EpisodeSubtitle`),不要留在调用点。
  留在调用点的话探针只钉得住格式化,钉不住「刮削不全的库上会不会变成空行」——
  实测把回落删掉,探针当场红;删之前它是绿的。
- 探针**只构造对象是不够的**:反向注入「`From` 不解析这三个字段」时它照样绿,
  因为断言全是直接 new 出来的 `CardItem`。补一条走 `From(ToJson(x))` 真往返的,
  才同时钉住解析和落缓存两头。

## 媒体信息:一行文字换成每条流一张卡(2026-09-12)

桌面草稿 03 页第 20 条(标着「已按你要求」):**每个版本一整块竖排,块内每条流
一张纵向卡片横排全铺开**(视频 / 每条音轨 / 每条字幕 / 常规),块内可左右拖动,
和 Emby 官端一致。此前这一块只有**一行文字**:

```
4K · HEVC · 18.4 GB · 3 条音轨 · 2 条字幕
```

「3 条音轨」说不出哪条是日语、哪条是国配;字幕是不是外挂更看不出来 ——
想知道只能起播之后到播放页去翻。`emby.itemMedia` 的 `streams` 里
这些字段一直都是全的(`core/emby/mediainfo.go` 的 `StreamInfo`,17 个字段)。

三条口径:

- **缺值整行不画**,不写「未知」。反向注入「缺值写未知」当场红。
- **外挂字幕必须标出来**:它是另一个文件、要单独 `sub-add` 挂载,
  行为和内封不是一回事(见 [[emby-external-subtitles]] 那条)。
- **声道优先写 `channel_layout`**(`5.1`),没有才回落到 `channels`(`2 声道`)。
  反过来的话 7.1 和 5.1 都显示成「6 声道 / 8 声道」,而那正是选音轨要看的东西。
- 卡高按**最长的那一块**统一算:各算各的会让这一排参差不齐,
  而看上去像是间距没调好。
...
## 分集三种版式:横排 / 网格 / 列表(2026-09-12)

草稿 03 页第 15 条要 `▦/☰`,而横排 + 左右翻页是用户 2026-09-03 自己点名的。
两边各要一半,那就三档都留 —— 横排是「接着看下一集」那个动作的版式,
网格 / 列表是「找第 300 集」那个动作的版式,不是同一件事。

三条实现口径:

- **网格 / 列表必须封顶高度**(`Carousel.Column` 的 `maxHeight`)。
  整页本来就在一个竖向 ScrollViewer 里,不封顶 = 外层用无限高去量,
  虚拟化面板把上千行全造出来。实测:去掉封顶那一句,800 行的盒子量出来
  **36794px 高**,而探针里「只造了 8 个」看着还像虚拟化生效 ——
  所以断言要同时判**高度**和**造了几个**,只判后者会假绿。
- **内嵌滚动区到底之后要把滚轮交出去。** `Smooth.Innermost` 原来判的是
  「滚不滚得动」,于是鼠标停在分集块上、块已经到底,整页就再也滚不下去。
  改成判「**还能不能往这个方向再走**」。日历 / 聚合 / 影视目录那三处内嵌
  滚动区一直有这毛病,一起好了。
- 版式选择存**静态字段**,不落库:点一集就是推一页新的 DetailPage,
  存实例上等于每点一集被打回横排;而它是「这会儿想怎么看」,不是设置项。
...
## 详情页头图改成通栏 24:8 剧照带(草稿 03 页第 12 / 13 条,2026-09-12)

原来是「海报 + 右侧一列(标题 / 小片 / 简介 / 播放键 / 媒体信息)」,背景大图
只是垫在这一列后面的一层淡淡的底纹。草稿画的不是这个:它是**一条通栏 24:8 的
剧照带**,带子里只回答「这是什么」——艺术字 + 一排元信息片压在左下,
左上浮返回,右上浮收藏 / 下载 / 更多⋯;播放键、简介、媒体信息全在带子**下面**。

改动里三件值得记的:

- **海报删了。** 草稿这一页就没画它。剧照已经是这部片的脸,旁边再摆一张 2:3 的
  同一部片,占掉三分之一宽度而没多说一个字。
- **高度必须自己算,而且要夹。** 24:8 在 1920 上是 640px —— 一条带子吃掉半个首屏。
  夹在 `[232, 420]`。另外**一张图都没有的条目不撑高度**(`Height = NaN`),
  撑着的话头顶是四百像素纯色,而「没刮图」是有些库的常态。
- **剧照走 ImageBrush 不走 Image。** Image 把自己的自然尺寸算进布局,
  一张 16:9 铺到 1600 宽就要 900 的高,带子当场被撑成两倍。画刷不参与测量。

自检 `LP_BAND=1`:量带子宽高、按 24:8 对账、数带子里的按钮和元信息片。
反向注入两次(高度写死 320 / 动作组不放进带子)都当场红。

## PowerShell 子进程会闪一个黑框,而且自检会拍到它(2026-09-12)

核心层为了读写 `.lnk` 起 `powershell.exe`。GUI 进程起控制台程序**默认会弹一个
控制台窗口** —— 用户看到的是「一进设置页黑框闪一下」。

它不只是难看:`scripts/shot-window.ps1` 按进程名找窗口,当场拍到了那个黑框,
截图变成 **237×39** 的一张小图。查了两轮才反应过来「截图为什么这么小」。

修法是 `SysProcAttr{HideWindow: true}`,非 Windows 一个空实现。
...
>
> 后一份多带了 `CreationFlags: CREATE_NO_WINDOW`(`0x08000000`)——
> 连控制台都不给分配,比只藏窗口更彻底。**没有单独验过只用 `HideWindow` 够不够**:
> 上面那次探 ffmpeg 的实测说明它对短命子进程是有效的,而覆盖脚本要挂到
> `Wait-Process` 超时(最长 120 秒),两者不是同一个场景,不能拿一个的结论替另一个签字。

## 媒体库:面包屑可点 + 已选筛选片(草稿 08 页第 8 / 10 条,2026-09-12)

两处都是「状态外显」:

- **面包屑「媒体库 › 某个库」**,点前一截回库列表。走的是新加的 `Nav.Top`
  **不是返回栈** —— 这一页也可能是从详情页的类型片跳过来的,栈上根本没有库列表,
  `Nav.Back()` 会退到详情页去。
- **已选筛选画成一行可去掉的片**(`Chips.Removable`)。原来只有三个下拉,
  「我现在筛了什么」得挨个读下拉框上的字;想去掉一条还得把那个下拉翻回「全部」,
  而没有任何东西提示它在哪。落地页那一档(类型/标签/工作室)画出来但**不给 ×** ——
  这一页存在的理由就是它。

自检 `LP_CHIPS=1`:选一个类型 → 该出「类型 · X ✕」→ 点 ✕ → 该一条都不剩。

☠ **第一版这条自检有洞**:它只判「片还在不在」。反向注入「✕ 只把片删掉、
筛选其实没变」时它照样绿 —— 因为片确实没了。补上 `_genre.SelectedIndex == 0`
才红。**判一个动作,就要判它真正改的那个状态,不是判它的显示副作用。**

## 截图脚本别信 MainWindowHandle(2026-09-12)

`.NET` 的 `Process.MainWindowHandle` 拿的是「第一个可见的顶层窗口」。鼠标停在界面上
弹出一个 tooltip 时,它指的就是那个 tooltip —— `shot-window.ps1` 截出来是一张
**237×39** 的小白图,而脚本照样报「成功」。连撞四次才反应过来「截图为什么这么小」。

改成 `EnumWindows` 枚举本进程的顶层窗口,取**面积最大**的那个;
「有弹窗就截弹窗」那一段也加了面积门槛(≥ 主窗的 20%),否则同一个 tooltip
会在第二步再把结果抢回去。

★ 另一条:**跑真机自检时机器不能有人在用**。用户在前台开着资源管理器时,
  截图拍到的是他的窗口,而功能自检(日志里那几行 ✓/✗)照样是准的 ——
  这两种证据要分开看,别因为截图不好看就以为功能坏了。

## 截图截出 237×39 小图:主窗恢复最大化的那一小段里它是隐形的(2026-09-13)

上一条的「取面积最大的顶层窗口」还不够。同一台机器上又连着截出三张 237×39 ——
`LP_SHOT_DEBUG=1` 打出来只有**一个**可见顶层窗口,而它就是那个 tooltip:
主窗在**恢复成最大化的那一小段**里 `IsWindowVisible=false`,压根没进枚举。
`LP_WAIT=45` 时同一条路正常(1920×1200),所以它是**时机**问题,不是逻辑问题。

修法:主窗有 `MinWidth=420 / MinHeight=360`,比这更小的一定不是它 ——
枚举不到够大的就每 500ms 再转一次,最多等 12 秒;等不到直接 `exit 3`,
不再交出一张「成功的」小图。

## 搜索从一张页改成全局浮层(草稿 09 页第 34 条,2026-09-13)

草稿要的是「Ctrl K 唤起、Esc 收起、**不跳页**」。原来它是侧栏里的一个大区,
点一下就把人从媒体库 / 详情页踢走 —— 而搜索是个**动作**,不是一个地方。

做法上有意识地避开了一个坑:**没有另写一套搜索**。浮层里装的就是原来那个
`SearchPage` 实例(`SearchHost` 那个 ContentControl),防抖、乱序丢弃、聚合分组
一行没动。另写一份的下场是其中一条路修了、另一条没修。

三处接线值得记:

- **侧栏那颗单选要弹回去。** 它仍然是 `RadioButton`(一堆代码按名字找它),
  `Checked` 里开浮层然后立刻把选中态还给上一项 —— 不还的话侧栏一直高亮在
  「搜索」上,而内容区显示的是别的页。
- **Esc 的优先级**:帮助浮层 > 搜索浮层 > 返回上一页。搜索开着时按 Esc 直接退页,
  人会莫名其妙被弹回上一页。
- **浮层不盖标题栏**(`Grid.ColumnSpan=2` 只盖侧栏 + 内容区)。盖了的话
  最小化 / 关闭那三颗按钮点不到。

空态摆**搜索历史片**:去重、置顶、封顶都在核心层(`config.ClampSearchHistory`
+ `prefs.pushSearch`),不在界面自己攒 —— 三端各攒一份的话「同一个词搜两次会不会
出两条」迟早说不一样的话。**搜不到的词不记**:历史是「回到刚才那次」的入口,
摆一个搜不到的词进去,点了还是搜不到。

☠ 自检的第一版反向注入选错了地方:把 `nav.search` 那条改回 `ShortcutNav`,
结果**照样绿** —— 因为侧栏的 `Checked` 处理器也会开浮层,两条路都通。
换成注掉 `Checked` 里那句才红(「跳页了:HomePage → SearchPage」)。
**注入要注在唯一的那条路上;有两条路时,注一条证明不了什么。**

## 媒体库版式开关 + 内容区右上两颗(草稿 08 页第 9 条 / 01 页第 5 条,2026-09-13)

### 列表版式不是「把卡片压扁」

两种版式回答的是两个问题:海报网格回答「有什么」(一屏尽量多的封面),
列表回答「这一条是什么」(标题一行读完,分辨率/码率/体积排成一列好比)。
所以 `MediaRow` 的标题**只给一行** —— 换行的话下一条的元信息就和这一条对不齐,
一列数字对不齐就没法比,那这个版式就白做了。

复用的两处:封面走 `Card.StartArt`(同一套骨架 / 淡入 / 失败收骨架),
右键走 `CardActions.Attach` —— **换个版式就少一半功能**是最不该有的差别。

版式落在核心层(`prefs.library_view`,写的时候拒未知值)。不落盘的表现是
「每进一次库都被拨回海报网格」—— 那颗按钮点了有反应,但选择留不住。
界面侧另记一份进程内缓存:每进一次库都等一趟偏好往返的话,网格会先按海报铺出来、
半秒后再跳成列表,那一跳比没有这个功能更难受。

收藏页那颗按钮**和媒体库共用同一份状态**(`GridView`,存档仍在 `prefs.library_view`)。
两边各存各的话,「在收藏页换成列表、回媒体库还是网格」这种分叉不报错 ——
用户只会觉得这个开关时灵时不灵。收藏页有**两个**网格(影片与剧集 / 分集),
所以断言判的是「一个海报卡都不剩」:只看第一个网格的话,
「分集那一栏没跟上」照样绿(实测注入 `FirstOrDefault()`,当场红成「列表行 3 / 还剩海报 3」)。
分集那一栏在列表里用 **16:9 缩略图** —— 集封面是横着的,塞进 2:3 的框里
`UniformToFill` 会左右各裁掉三分之一,而且画面是满的、不报错。

### 搜索 + 刷新放**外壳**,不放各页顶栏

十几页各摆一份的话,漏掉的那一页就是「这一页没有搜索」,而用户不会觉得那是设计。
所以那两颗挂在 `MainWindow` 的内容区 Panel 上,悬在正文之上。

配套三件:

- **刷新要有「怎么再造一个」。** `Nav` 的栈上每一层多存一个 `Func<Control>`,
  由压栈的人给。给不出的页(播放页)那颗按钮**不画** —— 摆一颗点了没反应的比没有更糟。
  刷新是**顶替**不是压栈:压栈的话按一次刷新就多欠一次返回。
- **滚下去要淡出**,而且淡了就不能再点中(`IsHitTestVisible=false`),否则是「点空气」。
  挂在 `PageHost` 一处收冒泡的 `ScrollChanged`,各页自己接必然漏掉自己 new
  ScrollViewer 的那几页。**要筛掉横向轨道**:不筛的话首页一横滚右上角就无故消失。
- **垫一层不透明底。** 窄窗口下正文的筛选条会伸到它底下,透明的话两层字叠在一起。
...
## 首登闸口:藏父容器拦不住任何东西(2026-09-16)

用户:「用户首次打开软件只有添加服务器页,不能有其他页面,防止误触导致软件流程卡住,
然后无法进行下一步,只能重来」。

闸口**本来就写了** —— `BootAsync` 里把 `NavList` 和 `ServerSection` 整条 `IsVisible=false`。
但它一个都没拦住,三个洞:

| 洞 | 为什么漏 |
|---|---|
| `Ctrl+1` / `Ctrl+L` 等导航键 | `ShortcutNav` 判的是 RadioButton **自己**的 `IsVisible`。**Avalonia 的 `IsVisible` 不继承** —— 藏掉的是父容器 `NavList`,每个 RadioButton 自己那一份仍是 `true` |
| `Ctrl+K` 搜索浮层 | `OpenSearch()` 一道判断都没有 |
| 内容区右上那颗「搜索」 | `SyncTools` 只为播放页让位,没管闸口 |

而**走掉之后回不来**才是「只能重来」的真因:闸口用的是 `Show()`,它直接写 `PageHost.Content`、
**绕过返回栈** —— 栈是空的,`Nav.CanBack` 恒 false。点一下设置(闸口下唯一留着的入口,
因为代理配错了就连不上任何服务器)走的是 `Nav.Root`,连栈底都换掉,于是:
侧栏藏着、返回键灰着、右上角没有入口 —— 整个软件当场没有出口。

修法是一个 `_gated` 布尔:`ShortcutNav` / `OpenSearch` / `SyncTools` 各查一次;
闸口改成 `Nav.Root(Gate(), Gate)` 压成栈底;设置在闸口下用 `Nav.Push` 而不是 `Nav.Root`。

> 通例:**「藏起来」不等于「拦住了」。** 凡是靠可见性做的门禁,都要去问一句
> 「有没有第二条不看可见性的路」—— 快捷键、深链、自检钩子、程序化 `IsChecked=true` 都是。

## 集详情页的选集栏(2026-09-16)

用户:「PC 端集详情页没有选集栏,无法点击观看记录里面的集在里面选集数」。

`DetailPage` 只认 `Series` / `Season` 两种类型去拉分集,`Episode` 两个分支都不命中 ——
于是 `_episodesHost` **根本没被加进 body**,不是「画了但空」。
而 2026-09-04 之后从首页「继续观看」点一集进的**正是集详情页**,
也就是说主路上反而没有选集。

修法:`EpisodesParent()` 一处判——剧/季用自己的 id,分集用 `series_id`。
传 `series_id` 不是 `season_id`:拿回全剧各季,`Episodes()` 自己 `GroupBy(SeasonNo)` 分季,
季选择器直接可用,和从剧详情页进来完全同构;传季 id 只有本季,跨季换不过去。

**同一套代码 `PlayerPage.LoadEpisodes` 早就这么做了**(它的注释里原话:
「集详情页手里没有分集表……而后者才是主路」)—— 播放页的选集面板对,详情页漏了。
缓存键(`PaintEpisodesFromCache`)必须用**同一个** parent id,用错了不报错,只是缓存永不命中。

## 屏蔽只对媒体库,而且被屏蔽的卡要留在页面上(2026-09-16)

用户:「首页的右键屏蔽条目不知道你是哪里看到的,Emby 官方网页端都没有这个东西。
屏蔽只出现在媒体库页:屏蔽某个媒体库之后不参与检索,该媒体库卡片样式变灰,
再次右键选择恢复即可恢复」。

- 条目卡的「屏蔽这个」**整项删掉**(不是关开关 —— `Features` 里那个 `card.block` 挡的只是菜单项,
  而这次是口径变了)。
- 库卡换一份**只有一项**的菜单:库不能播、不能标已看、不能收藏,那几项摆上去点了只会报错。
  判据 `IsLibrary(type)` 和 `LibraryPage.OpenDetail` 里那个分支必须一致。
- 「变灰」= 压暗 + 「已屏蔽」角标,**卡照样画出来**。整个藏掉的话屏蔽就是单向门,
  再也解除不了(Rust 版栽过,所以 `emby.views` 一直有 `include_blocked`)。
- 灰是**建卡那一刻画上去的**,不是绑定 —— 所以点完屏蔽必须 `Nav.Reload()`,
  否则界面毫无反应,而那和「点了没生效」长得一模一样。
- 名单要在**建卡之前**拿到,而且要排在缓存先行**前面**:排后面的话缓存命中那一版画的是
  不灰的卡,而内容没变时下面直接 return,灰永远补不上。

顺带补了右键的「查看详情」和「转到剧集」——列表版式和分集行的左键是直接起播,
原来想看简介根本没有入口;`CardItem` 为此补了 `SeriesId`(列表命令一直在发,只是没人接)。

## 「菜单里没这一项」其实是「点了看不出生效」(2026-09-16)

用户:「首页右键还是缺选项,标记为已观看、未观看那些呢」。

去数了一遍:「标记为已播放 / 未播放」在 `CardActions.Build` 里是**无条件加**的,
收藏、下载、查看详情也都在 —— 一项都不缺。缺的是**反馈**:

```csharp
public static void Attach(Control host, CoreClient core, CardItem item, Action<string>? after = null)
private static async Task<bool> Run(..., Action<string>? after) { ...; after?.Invoke(""); }
```

全仓三个 `Attach` 调用点(两处 `Card`、一处详情页分集行)**一个都没传 `after`**。
于是点「标记为已播放」:命令真发了、Toast 也弹了,而**卡片纹丝不动** ——
绿勾、未看数、收藏星标全是**建卡那一刻**画上去的,不重建就不会变。
下次再看还是没勾,用户自然认为「这个功能不存在」。

修法是给 `Attach` 一个默认 `after`:成功就 `Nav.Reload()` 重建当前页。失败不重建 ——
Toast 已经说了原因,重建一页只会把它抹掉。

☠ 屏蔽那一条**必须传 `null` 而不是走默认**:默认 `after` 会在 `Run` 返回时立刻重建,
而那时 `BlockedLibraries` 还没更新 —— 重建出来仍是不灰的卡,紧接着又重建一次。
顺序有要求的动作自己收尾。

> 通例:**可选回调的默认值是 `null` 时,要去数一遍「所有调用点都传了吗」。**
> 一个都没传 = 这个参数从来没生效过,而它不报错、不崩,只是功能少了一半。
> 同一形状的还有 `Features.On(...)`:开关关着和代码不存在,在用户眼里一模一样。

另外把剧 / 季上那一条的文案改成「标记**整部 / 整季**为已播放」:
核心层打的是 `POST /Users/{uid}/PlayedItems/{itemId}`,Emby 对 Series / Season
会把底下所有分集一起标上。写「标记为已播放」而实际标掉了 40 集,
用户不会知道自己刚干了什么,更不会知道还能一键标回来。

## 封面首次失败不重试 = 「随机有些封面加载不出来」(2026-09-16)

`Images.Fetch` 取不到就 `return null`,而卡片只在**构造那一刻**取一次。
于是上游抖一下(502 / 连接被掐 / 一屏几十张抢并发名额超时),那张卡到用户离页为止
都是占位图;重进一次又是好的 —— 看起来就是「随机」。

改成重试两次、退避 300/600ms,**但不重试 404**:那是 src 不在白名单或上游真没有这张图,
没刮削封面的库很常见,为它们空转是白烧并发名额。
判据要从响应里取(`StatusCode != NotFound`),不能只看「拿没拿到位图」——
两种失败在调用方眼里都是 `null`。

## 设置页:分大节 + 锚点条,不要左右分栏(2026-09-16)

用户:「PC 端的设置页样式很丑,优化样式和优化布局」。现状是 14 张一模一样的卡直上直下
堆成一列(同底色、同圆角、同内边距、同标题层级),找「日志」要滚到第 14 张。

**不能改成左侧导航 + 右侧内容** —— 2026-09-04 用户否过一次:「不要做成一列一列
导致视线需要从左往右拉很长去对齐」。这次他选的是「保持单列,但分大节 + 顶部锚点条」。

- 五节:常规 / 播放 / 弹幕与字幕 / 网络 / 高级。**空节整节不画**(功能开关关掉之后
  留一个只有标题的空节,比没有这一节更让人困惑)。
- 大节标题要**比卡标题更小更淡**再加一条分隔线。卡标题是 `h2`(16/SemiBold),
  大节再用同一档就读不出层级 —— 分隔线才是「这里换了一节」的信号,字号大小只是噪音。
- 锚点条放在**滚动容器外面**(`DockPanel` 的 Top),不做 sticky:
  sticky 在 `ScrollViewer` 里要自己算偏移量,放外面天生就一直在。滚动用 `BringIntoView()`。
- 大节间距 26、卡间距 14。两处同一个数的话分节就白分了。

顺带治了「同一页控件不是一套」:全仓 `*.axaml` 里 **CheckBox / ComboBox 一条样式都没有**,
跑的是 Fluent 出厂皮,行高 30/32/34/40 四种混着。加两条**按类型**选的全局样式
(不加 class —— 加了就得回去给十几处各挂一次,漏掉的那处就是「这一行和别的不一样」)。
**只设控件自己的属性,不去覆盖 Fluent 的内部资源键**:键名对不上时样式静默不生效,
而「设了没生效」比「没设」更难查(`TextBox.field` 那两条注释记着同一个坑)。
勾选框内部那个对勾的配色因此保持默认 —— 要改它得先把 Fluent 的资源键逐个实测一遍。

## 播放页的锁定键:白字透明底只在控制条上成立(2026-09-16)

用户:「PC 端播放页的锁屏按钮非常难看清,深色模式完全看不清」。

两处错叠在一起:① 用的是 emoji(🔓/🔒),走 `FontFamily.Default`,和 OSD 上其它按钮
那套 MDL2 线性图标不是一个东西;② 挂的是普通 `.osd` —— **白字 + 透明底,只有悬停才上白蒙版**。

`.osd` 那一套能成立,是因为其它按钮都待在上下两条控制条上,控制条自带深色渐变托着。
而锁定键悬在**画面正中偏左**,底下直接是片子本身:亮场景白字糊在白画面上,
暗场景连那层 `#33ffffff` 的悬停底也几乎看不见。

修法:换 MDL2 字形 + 一个 `.osd.scrim` 类(常驻深色蒙版 + 发丝白边),悬停只把底加深一档。
新样式要放在 `.osd` 那几条**之后** —— Avalonia 的样式是**后匹配的赢**,不是 CSS 那套权重。

## 2026-09-18 这一轮界面口径

- **搜索是一页,不是浮层 + 右上角常驻按钮**。用户:「很丑 而且一直放在那里 挡住了画面」。
  内容区右上那块(搜索 + 刷新)整个删了,刷新留在 F5 / Ctrl+R。浮层、Esc 收浮层、侧栏弹回那套都一起删了。
- **chip 一律圆角 6,不要胶囊**。用户:「有些按钮是矩形 有些按钮是圆角 统一矩形吧」。
  改的是 `Button.chip` 这一条样式 + 首页大图上的标签,不是逐个调用点。
- Fluent 的 ComboBox 悬停 / 按下是写在**模板里那块 `Border#Background`** 上的,`ComboBox:pointerover`
  设 BorderBrush 管不到它 —— 表现是悬停那个冒出一圈亮白框,和旁边的看着像两种控件。
- 播放页选集改成**贴右边的一栏**(`EpisodeDrawer`):封面 + 「第 N 集 标题」+ 「分辨率 / HDR·DV / 码率 / 大小」。
  动态范围来自核心层新字段 `Item.video_range`(列表条目加字段要同步改 7 条差分对账语料的 expect)。
- 追剧日历:七列横滚看板 → 「周几日期条 + 当天海报墙」;排行榜:两个下拉 → 两排 chip,前三名大卡。
  排行榜本地构建没有凭据,版式靠 `LP_RANKFAKE=1` 的假榜单看。
