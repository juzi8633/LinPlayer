...
☠ **核心层还有第二道闸,它比滤镜自己的阶梯快得多。** `guardInterp`(`core/player/interpcmds.go`)
起播 **2 秒**丢帧过 25% 就撤、8 秒过 10% 就撤,而滤镜从默认档降到地板要十几个源帧。
慢机器上那十几帧的卡顿足够触发那道闸 —— **阶梯还没来得及工作,功能已经被拔了**。
所以滤镜改成**从最省的一档起步再往上爬**:往上爬一档只要一个源帧,爬到顶半秒,
起播那一段先保证不卡。EWMA 也要拿第一帧实测值播种,从 0 爬的话开头几帧均值恒偏低,
会一路往上爬档,爬完才发现跟不上,等于自己制造了一段卡顿。

用户看到的提示能区分是哪道闸:
「补帧脚本跑不起来…mpv 的原话:显卡算力不够(每个源帧平均要 N ms)」= 滤镜自己降到底还不够;
「显卡跟不上补 N 倍(X 秒丢了 Y 帧)」= 丢帧闸;「补帧没有生效(播了 15 秒…)」= 起播闸。
...
## 片头片尾改成「填时长」,片尾按每集最后几秒存(2026-09-18)

- 用户:「不是让用户从这到这,而是给用户一个输入栏,让用户输入跳过的时长,这样用户点一下就可以跳过了」。
  弹层现在是「片头 从 [0:00] 开始 长 [90] 秒 / 片尾 最后 [120] 秒」。输入认 `90` 和 `1:30`。
- **片尾不能存绝对时间**:每集长短不一,第 1 集量的 23:10 放到第 2 集可能就在正片里。
  `SkipRange.OutroLen` 非 0 时按 `runtime - OutroLen ~ runtime` 算(`fillSkip`),压过老的 OutroStart/End。
- **手动填过的不看开关**:设置页「跳过片头 / 片尾」两个开关只管服务端章节和联网那两层。
  原来开关关着时 `fillSkip` 第一行就返回,于是用户填完、下一集跳过键不出来 —— 填了等于没填。
- 播放页的键盘处理器挂在**隧道**阶段(为了压过按钮吃空格),所以它比输入框先拿到键:
  在时长框里打数字 = 跳到百分之几,打空格 = 暂停。`OnKey` 开头按 `e.Source is TextBox` 放行 ——
  看 Source 不看焦点:弹层是另一个顶层窗口,页面的 FocusManager 看不见里面的焦点。

## 播放页的窗口拖动、三颗窗口键、字幕两行(2026-09-18)

- 播放页进沉浸式时连标题栏一起收了,窗口化看片时**拖不动也关不掉**(只剩 Alt+F4)。
  现在顶栏空白处按住拖(`BeginMoveDrag`)、双击最大化;顶栏右边补了最小化 / 最大化 / 关闭;
  窗口化(非全屏非最大化)时四周一圈 1px 发丝边。
- 字幕 / 音轨弹层两行:第一行轨道名(压制组写的 title,没有才用语言),第二行 `格式 · 语言 · 外挂 · 默认`。
