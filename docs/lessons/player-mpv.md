...
## 弹幕渲染器:SPIKE 原型和成品差在哪(2026-09-09)

`core/player/danmaku.go` 那一版是 SPIKE-5 的产物,能上屏但**不是能用的弹幕**:

- 分轨是 `lane = i % 16` —— **轮着来,不避让**。同一时刻的两条会稳稳叠在一行上。
- 字号写死 `\fs40`、滚动写死 8 秒、置顶置底写死 `lane % 3`,九项设置一项都没有。
- 颜色 `\c&H%06X&` 直接填 RGB。**ASS 的颜色是 BGR** —— 红蓝是反的。
  白色和灰色换不换都一样,所以这个 bug 能活很久;判据必须用**纯红**才抓得到。
...
- **行数的 0 是合法值**(= 这一类不显示),不能和别的数值项一样「0 当没设、回默认」。
  这一条最容易写错,而写错的表现是「置顶行数拉到 0,顶部弹幕照样满屏」。
- ASS 的 `\alpha` 是**透明度**:`00` = 完全不透明。面板上给的是不透明度,
  换算是 `(1 - opacity) * 255`,方向反了的表现是「拉到 0% 反而最清楚」。
- 一轨的再用时机 = **本条尾巴进画面的时刻**(`t + w/(resX+w)*roll`),
  不是「整条走完」。用后者的话一轨一次只放得下一条,屏幕会空得离谱。
- 文本宽度只能**估**:精确宽度得问 libass,而 `osd-overlay` 这条路拿不到测量结果
  (`compute_bounds` 是另一套 API,走它每帧多一次同步等待)。估宽只影响「一轨塞多密」。
