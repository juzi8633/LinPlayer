...
电视端手机扫码遥控**不挂在这里**:它是 `core/companion` 的独立监听器,绑局域网网卡随机端口,
路径 `/c/<token>`(见 `UI_TV.md` §9.3)。挂进来就得让本服务绑 `0.0.0.0`,图片代理等路由会一起暴露到局域网。

### 安全约束

- 只绑 `127.0.0.1`。
- 所有路由都要鉴权,**但鉴权方式分两种**(见下)。
...
| **D. DirectComposition 视觉合成**(Windows) | mpv 用 `gpu-api=d3d11` 渲进自己的 swapchain,UI 走 Avalonia `WinUIComposition`,两个 visual 放进同一棵 DComp 树 | Windows 专属;需要 Avalonia 合成模式与 DComp 互操作;Linux 要另想 | **唯一可能同时拿到 单窗口 + 自绘 OSD + 零拷贝 的方案** |
| **A2. 双顶层窗口**(现状) | mpv 独立顶层窗口在下,透明 UI 窗口在上,手动对齐几何 | 几何对齐的一长串坑(全屏白边 / z 序漂移 / wndproc 重入 / DPI) | **已在产验证可行**,是保底方案 |

#### 【已重新裁决】路径 B 可用,但要拿 CPU 换零拷贝

> v0.1 写「默认赌 B」;2026-08-31 第一版实测把它推翻了,**但那次实测有测试台污染,
> 结论是错的**。同日重测订正。全过程与五个踩坑见
> [`spikes/SPIKE-1b-zero-copy.md`](spikes/SPIKE-1b-zero-copy.md)。

**两条结论:**

1. **Windows 上路径 B 拿不到 `d3d11va` 零拷贝** —— 只要渲染走 OpenGL 就跨了 D3D11 设备边界,
   只能 `d3d11va-copy`。mpv 自己的 `gpu-context=win` / `dxinterop` 也一样;
   **只有 `gpu-api=d3d11`(即路径 A)拿得到。**
2. **但退化后的解码够用。** 实测(Intel UHD 770 核显,输出 1920×1080):
...
- **4K60 是这块核显本身的天花板,不是 B 独有的** —— 零拷贝的 A 在同机同片下丢帧数相近
  (407 vs 506)。且实拍 4K 内容基本是 23.976/24/25 fps。
- **B 那一栏含测试台开销**(Python 忙轮询 + 无垂直同步)。同解码路径下 mpv 自有 GL VO 只要
  **12%**,真实实现的期望值应接近它。**这是估计不是实测。**

#### 方案 D(DirectComposition)已排除

原设想让 mpv 渲进一个可被外部合成的 swapchain,由 DComp 与 UI visual 合成。
**实测排除:libmpv 的公开 API 给不出 swapchain** —— render API 只有 `OPENGL` / `SW` 两种类型,
`vo=gpu` 系列自己拥有 HWND 并自己呈现;`--d3d11-composition` 这个选项**不存在**
(探测返回 option not found)。

> **澄清一个名字上的误会:** Avalonia 的 `WinUIComposition` **不是 WinUI**。
> 它是 Win32 后端的**呈现模式**,底层是 `Windows.UI.Composition`(DirectComposition 的现代版),
> 属于操作系统合成器 API,不是 UI 框架。**路径 B 与 A2 都不需要碰 WinUI。**
...
**注意 A1 从来不是候选** —— 所以「OSD 好看」与「解码流畅」**从来不在同一条取舍轴上**;
真正的轴是「几何对齐的复杂度 vs 每帧一次 copy-back 的 CPU」。

#### 【已确认并量化】B 在 Windows 上拿不到 d3d11va 零拷贝 🔴 R14

初测(WGL)怀疑是上下文没配对,**换成 ANGLE 后复测,仍然拿不到** ——
已确认是结构性的,不是配置问题。数字与根因见上方「已推翻」小节与
[`spikes/SPIKE-1b-zero-copy.md`](spikes/SPIKE-1b-zero-copy.md)。
...
### 7.5 【决策】弹幕:核心层排版,两端 UI 绘制

> **v0.3(2026-09-10)推翻了 v0.2 的「走 `osd-overlay`」。** 代码已按 v0.3 落地,
> `osd-overlay` 那条路整段删除。下面保留 v0.2 的论证和 SPIKE-5 实测数据 ——
> 推翻它的正是那份数据里「仍未回答的」那一条。
>
> **v0.3 的结论:** 排版(合并 / 分轨 / 丢弃)留在核心层,一次算好由
> `player.danmakuLayout` 整份交出;绘制在两端 UI(安卓 Compose `Canvas`、
> 桌面 Avalonia `Control`),各自用本地帧回调插位置。
>
> 三条推翻理由:
> ① **Exo 内核下 `osd-overlay` 根本不存在**,而 `player.danmakuSet` 照样返回成功;
> ② 平滑度被钉在 19 次/秒(见下表),而 UI 层的帧回调本来就是 60Hz;
> ③ 每秒 120 条 overlay 命令排在 mpv 的核心线程上,和解封装、解图形字幕抢。
>
> v0.2 表里「三端各写一个 Canvas 等价物 = 三份新 bug」那一条**只在把排版也复制三份时成立**。
> 排版留在核心层之后,两端各自只剩「插 x + 画字」,那不是三份实现,是一份数据两个画笔。
>
...
- TV 形态:`androidx.tv.material3` + Compose 焦点系统。**完整规格见 [`UI_TV.md`](UI_TV.md)**。
...
**「一切持久化」的唯一例外:纯呈现的、只属于这一台设备的偏好。**
深浅色覆盖是目前唯一一项 —— 它**不该**跨设备同步(手机上强制深色不代表电视上也要),
和「我在哪一页、滚到哪」是同一类东西。这类值存各端本地
(安卓在 `data/UiPrefs.kt` 的 SharedPreferences)。

判据只有一条:**核心层有没有消费点**。有消费点的一律归核心层 ——
放进本地的那一刻它就变成「设了但核心层不知道」的开关,而那是本仓最难查的一类 bug。
【用户定 2026-09-06:不做账号系统,主题存本地就行】
...
| `cache/img/` 磁盘层 | **2 GB** | LRU,按 mtime;满了删到 90% |
...
> 与黄金实现 `crates/core/src/image_cache.rs:27,29` 一致。
> **本表早先写的「默认 512 MB」是错的,且漏了内存层**,2026-08-31 订正。
...
4. 线路页显示名称 + 地址 + 延迟(用户 2026-09-06 改口);别处仍不展示地址
